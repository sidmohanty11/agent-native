...
<Diagram id="doc-block-13zbuxe" title="A Claude-Code setup, but stored as agent resources in SQL" summary={"The same customization layer — instructions, skills, memory, agents, jobs, MCP — except every file is a row in a shared multi-tenant database."}>
