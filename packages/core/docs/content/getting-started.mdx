...
To author your own chart instead of using the built-in data widgets, give the
action a product-specific renderer id and return your own validated JSON shape:
...
Then register a React component for that exact renderer id from app startup, for
example by importing `app/chat-renderers.tsx` once from `app/root.tsx`:
