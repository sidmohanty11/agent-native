...
<Diagram id="doc-block-sntrhp" title="Unified deploy: one origin, one path per app" summary="Every app ships behind a single origin, so login sessions and cross-app A2A are free.">
