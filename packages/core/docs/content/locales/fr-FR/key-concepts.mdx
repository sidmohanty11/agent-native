  ...
  <div class="diagram-row">
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >lit et écrit des données, exécute des actions, modifie le code</small
      >
    </div>
    <div class="diagram-card">
...
(`databaseTools: "read"`), donc les agents inspectent les données avec `db-schema` /
`db-query` et écrivent via les actions typées de l'application. Définissez
`databaseTools: "write"` (ou `true`) seulement pour les surfaces de maintenance
qui doivent exposer `db-exec` / `db-patch`; utilisez
`databaseTools: "off"` / `false` pour exiger des actions typées pour toutes les données
accès.
