...
<Diagram id="doc-block-dkfzdz" title={"Le principe de l'échelle"} summary={"La plupart des équipes s'arrêtent au niveau 1 ou 2. L'agent natif est le niveau 3 : une véritable application et un véritable agent sur une surface d'action partagée."}>

```html
<div class="diagram-ladder">
  <div class="diagram-card rung rung-3">
    <span class="diagram-pill accent">Échelon 3 · agent-native</span
    ><strong>Agent + UI comme partenaires égaux</strong
  ...
  <div class="diagram-card rung rung-2">
    <span class="diagram-pill">Échelon 2</span
    ><strong>Un chat avec des outils</strong
    ><small class="diagram-muted"
      >L’agent peut agir, mais ce n’est encore qu’une fenêtre de chat. Pas de
      tableaux de bord, listes ou raccourcis.</small
    >
  </div>
  <div class="diagram-card rung rung-1">
    <span class="diagram-pill warn">Échelon 1</span
...
<Diagram id="doc-block-qsmzjp" title={"Un système, deux façons d'entrer"} summary={"L'agent et l'interface utilisateur écrivent dans les mêmes actions et dans la même base de données. Quoi que l’un fasse, l’autre le voit."}>
...
<div class="diagram-parity">
  <div class="diagram-col">
    <div class="diagram-node">
      Human<br /><small class="diagram-muted"
        >clics, formulaires, raccourcis</small
      >
    </div>
    <div class="diagram-node">
      Agent<br /><small class="diagram-muted"
        >langage naturel · A2A · Slack</small
      >
    </div>
  </div>
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">base de données SQL</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-box">L’UI se met à jour en direct</div>
</div>
```
...
Agent-native offre à chaque utilisateur la même couche de personnalisation, sans jamais quitter l'application. Chaque application est livrée avec des **ressources de l'agent** personnelles que vous (ou n'importe quel membre de votre équipe) pouvez utiliser pour :
...
Le problème : tout est stocké dans la base de données, pas dans le système de fichiers. Il n'y a pas d'environnement de développement à démarrer, pas de conteneur par utilisateur. Chaque utilisateur dispose de son propre ensemble complet de ressources de l'agent (mémoire personnelle, connexions personnelles, skills personnel) essentiellement gratuitement, car il s'agit de toutes les lignes d'un tableau. C'est ce qui rend la flexibilité au niveau du code Claude viable dans un véritable produit SaaS multi-tenant.

Voir [Ressources de l'agent](/docs/agent-resources) pour le concept complet.
  ...
  annotations={[
    {
      ...
      note: "`description` est ce que l'agent lit pour décider quand l'appeler comme outil.",
    },
    {
      lines: "6",
      label: "Contrat typé",
      note: "Un zod `schema` valide les entrées de **chaque** surface — agent, UI, HTTP, MCP et A2A.",
    },
    {
      lines: "7-10",
      label: "Une mise en œuvre",
      note: "Le corps `run` est la seule source de vérité. Le bouton de l'interface utilisateur et l'outil d'agent exécutent tous deux exactement cela.",
    },
  ]}
/>
...
- [**Ressources de l'agent**](/docs/agent-resources) — la couche de personnalisation par utilisateur (skills, mémoire, instructions, MCP) soutenue par SQL, pas des fichiers
