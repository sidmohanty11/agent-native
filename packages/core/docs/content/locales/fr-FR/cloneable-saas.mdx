...
<Diagram id="doc-block-1c2xd9a" title="Fourcher et personnaliser" summary={"Choisissez un produit fini, marquez-le, faites-le évoluer dans un anglais simple et expédiez-le vers votre propre domaine."}>
...
<Diagram id="doc-block-1jgvf3z" title={"Pourquoi les forks par utilisateur évoluent"} summary={"Deux idées maintiennent le modèle fork-and-customize pratique : l'agent effectue la maintenance et la personnalisation par utilisateur réside dans SQL – et non dans le code par utilisateur."}>
