...
<Endpoint id="doc-block-173hyrp" method="POST" path="/_agent-native/mcp" summary={"Point de terminaison de passerelle MCP unifiée"} auth="Standard remote MCP OAuth, handled by the framework. The granted-app set scopes which workspace apps the connector can reach." responses={[
  {
    "status": "200",
