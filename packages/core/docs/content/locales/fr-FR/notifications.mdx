...
<Endpoint id="doc-block-1wwo0xz" title="Liste des notifications" summary={"The route behind listNotifications() — scoped to the authenticated session's email."} method="GET" path={"/_agent-native/notifications?unread=true&limit=50"} summary={"Répertorier les notifications récentes pour l'utilisateur actuel"} auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
