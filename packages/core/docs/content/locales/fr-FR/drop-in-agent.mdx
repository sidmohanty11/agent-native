...
C'est tout. L'utilisateur dispose désormais d'un agent commutable sur chaque page – avec historique des discussions, onglet ressources de l'agent, terminal CLI, saisie vocale et mode plein écran. L'état persiste lors des rechargements via `localStorage`.
