...
- **Mentions de canal** : utilisez une @mention pour démarrer le travail. Les réponses sans mention ne continuent que tant que ce fil précis contient un travail actif.
...
OAuth géré est la configuration recommandée. Dans **Paramètres → Messagerie**,
...
`message.channels`, `message.groups`, `message.im` et `message.mpim`.
Configurez `SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET` et
`SLACK_SIGNING_SECRET`, puis cliquez sur **Add to Slack** dans
**Paramètres → Messagerie**. Chaque installation conserve son jeton chiffré et
...
Les réponses sans mention ne pilotent qu'un fil dont la tâche est active ; les
autres messages du canal n'appellent pas l'agent. Un contexte borné charge les
...
| Cela a marché une fois, puis une réponse ultérieure dans le fil a été ignorée | C'est normal. Les réponses de fil sans mention n'atteignent l'agent que tant que ce fil a un travail actif — refaites une @mention.                            |
