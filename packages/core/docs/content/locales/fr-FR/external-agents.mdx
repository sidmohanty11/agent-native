...
| `ask_app({ app, message, async?, maxWaitMs? })`    | boucle d'agent     | achemine une tâche en langage naturel vers l'agent intégré à l'application ; les tâches longues renvoient un `taskId` durable pour l'interrogation via `ask_app_status` |
