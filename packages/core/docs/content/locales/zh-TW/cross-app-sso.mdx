...
1. **應用程式→調度（授權）。**應用程式將使用者傳送到身分授權機構：
...
4. **應用程式透過電子郵件驗證和 JIT 連結。**應用程式使用自己的 `A2A_SECRET` 驗證權杖簽名，檢查 `scope: "identity"` 和 `exp`，然後嚴格透過經過驗證的電子郵件執行**即時連結**：
