...
Agent-Native 是一個用於建置應用程式的框架，其中 AI 代理及其週圍的產品表面是平等的合作伙伴。該介面可以從一個無頭代理開始，透過一個自訂操作，發展成為豐富的聊天，或者成為一個完整的 UI。不變的是智慧體和人類共用相同的 actions、資料庫和狀態。完整解釋請參見 [What Is Agent-Native?](/docs/what-is-agent-native)。
...
- **如果您想要在提交到 UI 之前執行一個操作和一個無頭應用程式代理迴圈，請先啟動原語優先** — 從 [Getting Started](/docs/getting-started) 開始。
...
```an-diagram title="附加人工智慧與 agent-native" summary="螺栓固定的聊天側邊欄有自己的世界。 agent-native 代理與 UI 共用相同的操作、資料庫和狀態。"
{
