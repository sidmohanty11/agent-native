---
title: "擴充功能"
description: "使用者在範本內建置的迷你應用程式 - Analytics 中的自訂 KPI 磁貼、行事曆中的會議準備清單、郵件中的聯絡人 CRM 小工具。無需部署，無需編輯程式碼，無需更改架構。"
---
...
| `dbExec(sql, args)`                              | 寫入 SQL                                  | `dbExec('INSERT INTO ...')`                               |
...
```an-diagram title="插槽將脈絡推送到小工具中" summary="主機範本擁有命名槽；將擴充功能安裝到其中，可以為使用者目前正在檢視的任何內容提供 window.slotContext 。"
{
"html": "<div class=\"slot\"><div class=\"diagram-card\"><span class=\"diagram-pill\">郵件對話串</span><small class=\"diagram-muted\">slot <code>mail.contact-sidebar.bottom</code></small></div><div class=\"diagram-arrow diagram-accent\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-box accent\"><code>window.slotContext</code><br><small class=\"diagram-muted\">{ contactEmail }</small></div><div class=\"diagram-arrow diagram-accent\" aria-hidden=\"true\">&rarr;</div><div class=\"diagram-card\"><span class=\"diagram-pill\">聯絡人備註</span><small class=\"diagram-muted\">loads notes for that contact &mdash; same widget, different context</small></div></div>",
...
此框架允許代理直接編輯應用程式的來源程式碼——元件、路由、樣式。那麼您什麼時候應該尋求延期呢？
...
| **最適合**   | 個人小工具、自訂 KPI、每個團隊的實用程式 | 向所有使用者提供的核心功能 |

經驗法則：**如果它適用於一個使用者或一個團隊，那麼它就是一個擴充功能。**如果範本的每個使用者都應該獲得它，請將其作為一項真正的功能提供。
...
- **專用網路保護** - 擴充功能無法到達內部地址。
- **需要驗證** - 擴充功能僅針對登入使用者執行，並且 `dbQuery` / `dbExec` 呼叫是自動範圍的。
...
- [**Actions**](/docs/actions) — 擴充功能透過 `appAction` 呼叫的操作
