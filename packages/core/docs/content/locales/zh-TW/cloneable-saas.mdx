...
<Diagram id="doc-block-6un5oc" title={"分叉並定制"} summary={"選取一個成品，對其進行品牌化，用簡單的英語對其進行改進，然後將其傳送到您自己的域。"}>

```html
<div class="diagram-fork">
  <div class="diagram-card">
    <span class="diagram-pill">1</span><strong>Pick</strong
    ><small class="diagram-muted">a complete template</small>
  </div>
...
<Diagram id="doc-block-oqiktb" title={"為什麼每使用者分叉規模"} summary={"有兩個想法使分叉和定制模型保持實用：代理進行維護，每個使用者的定制位於 SQL 中，而不是每個使用者的程式碼中。"}>
