...
```an-diagram title="中心模型：一個應用程式為組織範圍的 MCP 伺服器提供服務" summary="Dispatch 擁有組織範圍 MCP 伺服器；消費者應用程式將它們拉取並合併為 mcp__hub_<orgId>_<name>__*。僅共用組織範圍的行 - 個人憑證保持不變。"
{
