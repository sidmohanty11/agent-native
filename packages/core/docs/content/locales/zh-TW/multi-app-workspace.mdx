...
> **哪個工作區檔案？** 此頁面涵蓋 **部署形狀** - 一個單一儲存庫、許多應用程式、共用驗證和統一部署。對於工作空間*是什麼*（自訂層：`AGENTS.md`、`LEARNINGS.md`、個人記憶、skills、自訂代理），請參閱 [Workspace](/docs/workspace)；有關治理（誰審查、核准和擁有什麼），請參閱 [Workspace Governance](/docs/workspace-management)。
...
```an-diagram title="三層，按檔名合併" summary="每個應用程式首先從應用程式本機解析外掛、技能、操作和 AGENTS.md，然後是共用包，然後是框架預設值。"
{
...
調度資源的範圍為**所有應用程式**（每個應用程式在執行時繼承它們，沒有複製或同步步驟）或**選定的應用程式**（為每個應用程式授予特定於應用程式的脈絡）。請參閱 [Workspace](/docs/workspace#global-resources) 了解完整的資源模型表、路徑約定和推薦的入門包。
...
- [Workspace](/docs/workspace) - 工作區中每個應用共用的自訂層（`AGENTS.md`、`LEARNINGS.md`、個人記憶、skills、自訂代理）。
