...
```an-diagram title="一名代理將工作交給另一名代理" summary="郵件代理發現分析代理的卡，傳送 JSON-RPC 訊息，並返回已完成的工作。"
{
...
```an-diagram title="無伺服器上的非同步工作生命週期" summary="async:true 在幾毫秒內返回工作，然後在呼叫者輪詢時重新執行執行代理迴圈。"
{
