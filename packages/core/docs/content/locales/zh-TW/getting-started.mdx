...
如果想建立自己的圖表，而不是使用內建 data widgets，可以給 action 一個產品專屬 renderer id，並返回你自己的已驗證 JSON 形狀：
...
然後在 app 啟動時為這個完全相同的 renderer id 註冊 React 元件，例如從 `app/root.tsx` 匯入一次 `app/chat-renderers.tsx`：
