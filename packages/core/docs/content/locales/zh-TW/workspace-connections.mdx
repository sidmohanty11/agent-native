...
```an-diagram title="連線一次、授予應用程式、重複使用憑證" summary="連線儲存提供者中繼資料（絕不是秘密）和指向保管庫的 credentialRefs。每個應用程式授予解鎖它。應用程式讀取單一就緒狀態。"
{
...
Dispatch 公開了控制平面 actions，該控制平面編寫應用程式可以直接呼叫的相同共用儲存函式：
