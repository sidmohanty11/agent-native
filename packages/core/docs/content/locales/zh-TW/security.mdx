...
```an-diagram title="層層防守" summary="此框架擁有大部分威脅面；您擁有兩件事——標記表以確定範圍和驗證外部輸入。"
{
...
```an-diagram title="範圍界定管道" summary="代理 SQL 從不直接接觸基表 - 它讀取範圍為目前識別的臨時檢視，因此裸表名稱只能返回擁有的行。"
{
...
`db-query` / `db-exec` 工具拒絕模式限定的表引用（`public.<table>`、`main.<table>`）——限定名稱解析為基表，並會繞過上面的臨時檢視。代理使用裸表名稱；範圍會自動應用。
...
入站 webhook 處理常式（Resend、SendGrid、Slack、Telegram、WhatsApp、Recall.ai、Deepgram、Zoom、Google Docs Pub/Sub）預設在正式環境中拒絕偽造請求：當缺少相應的簽名秘密環境變數時，處理常式返回 401，而不是接受和分派。
