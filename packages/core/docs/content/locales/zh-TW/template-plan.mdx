---
title: "視覺計畫"
description: "Agent-Native 計畫將您的編碼代理的計畫轉變為結構化的、可審閱的檔案 - 圖表、線框圖、帶註解的程式碼、註解和共用連結。從 CLI 安裝一次；您與之共用的審閱者以訪客身分進行編輯，登入後僅可儲存或共用。"
---

# 視覺計畫

> **大多數人會把 Plan 作為 skill 安裝，而不是 scaffolded app。** 一個 CLI 指令會把 `/visual-plan`、`/visual-recap`、`/visualize-repo` skills 和託管的 Plan 連線器新增到你的 coding agent — plugin 和 marketplace 路徑請參閱 [Plan plugin & marketplace](/docs/plan-plugin)。Fork Plan template（見 [For developers](#for-developers)）是用於自託管或基於 Plan 本身建置的次要路徑。

Agent-Native 計畫是編碼代理的視覺化計畫模式。它變成了普通
Codex、Claude 程式碼、Markdown 或將實施計畫貼上到結構化中
使用富文字、圖表、線框圖、帶註解的程式碼演練檢視表面
以及檔案樹、註解、評論和可共用連結。

它歸結為兩個指令。 `/visual-plan` 在代理之前**制定計畫
編寫程式碼。 `/visual-recap` 將**已經發生的變化變成了 PR，
提交、分支或 git diff — 進入高海拔視覺化程式碼審查。都開啟
相同的審閱介面，因此您可以註解、評論並將意見回饋返回給
以同樣的方式代理。

<Diagram id="doc-block-14bxxm9" title={"兩個指令，一個檢視介面"} summary={"這兩個指令都透過託管的 Plan MCP 連線器發布到相同的註解和評論介面。"}>
...
<div class="diagram-plan">
  <div class="diagram-col">
    <div class="diagram-node">
      ...
      ><small class="diagram-muted">寫程式碼前 — 架構、UI、重構</small>
    </div>
    <div class="diagram-node">
      <span class="diagram-pill">/visual-recap</span
      ><small class="diagram-muted">寫程式碼後 — PR、提交、分支、diff</small>
    </div>
  </div>
  ...
  <div class="diagram-node">
    編碼代理<br /><small class="diagram-muted">意見回饋已交回</small>
  </div>
</div>
```
...
<WireframeBlock id="doc-block-aglxh2">
  <Screen
    surface="desktop"
    html={
      "<div style='display:grid;grid-template-columns:1fr 250px;gap:14px;padding:16px;min-height:520px;box-sizing:border-box'><main style='display:flex;flex-direction:column;gap:12px;min-width:0'><div style='display:flex;align-items:center;gap:10px'><h1 style='margin:0'>結帳改版計畫</h1><div style='flex:1'></div><button>分享</button><button class='primary'>核准</button></div><div class='wf-card' style='display:grid;grid-template-columns:1fr 1fr;gap:10px;min-height:150px'><div class='wf-box'>目前線框圖</div><div class='wf-box'>擬議線框圖</div></div><div class='wf-card' style='flex:1;display:flex;flex-direction:column;gap:10px'><strong>實施計畫</strong><div class='wf-box'>決策：保留現有結帳外殼</div><div class='wf-box'>帶註解的程式碼演練</div><div class='wf-box'>開放問題</div></div></main><aside class='wf-card' style='display:flex;flex-direction:column;gap:10px'><strong>評論</strong><div class='wf-box'>主 CTA 上的標注</div><div class='wf-box'>給代理的問題</div><div class='wf-box'>已解決的文案備註</div><button class='primary'>交回意見回饋</button></aside></div>"
    }
  />
</WireframeBlock>
...
一步完成特定於用戶端的驗證/設定流程，因此您的第一個工具呼叫不會
撞到 OAuth 牆：
...
該指令會安裝三個指令：`/visual-plan`、`/visual-recap` 和 `/visualize-repo`。

如果您使用的是直接接受 MCP 連線器 URL 的基於聊天的主機
（而不是 CLI 設定的用戶端），連線託管計畫連線器
`https://plan.agent-native.com/mcp` — 請參閱 [MCP Clients](/docs/mcp-clients) 以了解特定於用戶端的設定。

驗證是在設定時進行一次性瀏覽器登入 - 這是有意為之的
是讓代理持續存在並共用其產生的計畫的原因。授權是什麼
步驟取決於您的客戶：

- **支援 OAuth 的主機**（Claude 程式碼）取得僅 URL 的 MCP 條目以及提示
  執行`/mcp`並選取**驗證**。
...
預設情況下，CLI 以它可以設定的每個受支援的本機用戶端為目標。透過
...
透過`--no-connect`註冊連線器而不進行驗證，然後執行
...
要自動生成**每個 pull request**的回顧，請傳遞 `--with-github-action`。
...
中繼資料還包括 VS Code 切換 URL：

```text
vscode://builder.agent-native/open?url=<encoded-plan-url>
```

擴充功能處理 URI，在 VS Code Web 檢視中開啟解碼後的計畫 URL，
...
- 應用程式的預設聊天模型是 GPT-5.6 Luna。直接連接 Anthropic 時仍預設使用 Claude Sonnet 5。你可以隨時在 **設定** 中變更模型；Haiku 是較便宜的 Claude 選項。
...
- **實施前審查。** React 到圖表、線框圖、選項分頁，
  開放問題表格、風險說明、帶註解的程式碼演練和程式碼
  在代理編輯檔案之前進行預覽。
- **直接對計畫發表評論。**將意見回饋固定到文字、圖片、線框或
  畫布位置；選取評論是針對代理還是針對人類
  審稿人； @提及有行內標籤的隊友；並將評論解決為
  計畫不斷發展。
- **清楚地將意見回饋意見回饋給代理。**文字評論附在最近的位置
  散文塊，視覺註解包括精確的目標中繼資料和瀏覽器
...
計畫行內散文編輯：點選進入任何文字部分，輸入豐富的格式
編輯器工具列或斜線選單，並且計畫自動儲存基礎降價。評論
註解模式暫時將文字部分變為唯讀，以便點選可以固定
意見回饋；離開審閱模式以繼續編輯散文。
...
未通過託管計畫 actions 傳送。保持橋接行程執行
審查； URL 是您電腦的本機連結，不是可共用的團隊連結。
serve指令預設將開啟的 URL 寫入`.plan-url`，因此編碼代理可以
捕獲它而不抓取長時間執行的標準輸出；將該檔案視為僅本機檔案
因為 URL 包含橋接權杖，所以不要提交它。

在 macOS 上，`--open` 更喜歡 Chrome/Chromium，因為 Safari 可以阻止託管
HTTPS 透過取得 HTTP 本機主機橋來計畫頁面。對於無頭
故障排除，執行：
...
本機規劃到託管資料庫中，用本機呼叫`publish-visual-plan`
MDX 資料夾路徑；這將上傳計畫，為其分配託管 ID，啟用共用
並評論，並返回託管的 URL。本機檔案模式沒有
自動使您的編碼代理的 LLM 本機化；選取本機或核准的
模型是否隱私邊界也很重要。

## Repository 視覺化文件 {#visualize-repo}

當你想為 codebase 建立一層持久的視覺化文件，而不是一次性的 plan 或 recap 時，使用 `/visualize-repo`。它會建立或開啟本機 Plan MDX workspace，在 `agent-native.json` 中更新 `apps.visualize-repo` 的 local-files 設定，並透過同一個 localhost bridge 提供文件：

```bash
npx @agent-native/core@latest visualize-repo --open
```

如果不存在 `agent-native.json`，該指令會引導產生一個，並根據 repository 結構推薦一個小的起始集合，例如 `actions/`、`app/components/`、`app/pages/`、`server/db/schema.ts`、`src/`、`packages/`、`templates/`、`docs/` 或 `content/`。你也可以指定具體的 source anchors：
...
起始資料夾預設位於 `.agent-native/visual-docs/repo-overview`。Comments 會保留在本機 `comments.json` 中，因此 reviewers 可以為 coding agent 標記章節，agent 也可以在同一次變更中同時更新 code 和 MDX。執行 `visualize-repo check` 做本機 lint；當你希望 Plan renderer schema 驗證 MDX 時，執行 `visualize-repo verify`。

## 桌面本機檔案同步 {#desktop-local-sync}

Agent Native Desktop 還為託管計畫提供了本機本機資料夾橋。這個
與本機檔案隱私模式不同：託管計畫資料庫仍然是
共用、評論、歷史紀錄和即時評論的真實來源，而桌面
...
已登入的所有者可以從計畫清單中刪除其託管計畫和摘要，或者
計畫操作選單。

- **軟刪除**將計畫移至**已刪除**分頁，製作正常計畫
  檢視/直接連結停止工作，並透過建立行來刪除公開存取
  私人。 SQL 行將被保留，以便所有者稍後可以恢復計畫。
- **恢復**可從軟刪除計畫的**已刪除**分頁中取得。
- **永久刪除**刪除託管計畫行和計畫範圍的評論，
  部分、活動事件、版本快照、共用授權、濫用報告和
  SQL 資產紀錄。 UI 需要在決賽前輸入 `DELETE <plan-id>`
  按鈕啟用。

永久刪除會刪除 Plan 應用的資料庫紀錄和 SQL 支援的資產
位元組/引用。如果部署使用外部上傳提供者，則提供者
...
- “執行 `/visualize-repo`，從我們的公開 API actions 和 schema 開始。”

## 從驗證錯誤中恢復 {#auth-errors}

如果計畫工具返回 `needs auth`、`Unauthorized` 或“工作階段”
已終止`，不要繼續重試。使用
`npx -y @agent-native/core@latest reconnect https://plan.agent-native.com --client codex`對於 Codex，或在支援 OAuth 的主機中重新執行`/mcp` → **驗證**。開始一個
在等待該工具之前新建 Codex 對話串或重新啟動/重新載入相關用戶端
要更新的註冊表。
...
| `plan_assets`      | 內嵌圖片資源儲存為base64（沒有上傳提供者時的回退）                                                                                                                                |

<DataModel
  id="doc-block-6c4jyi"
  title={"規劃資料模型"}
  summary={"一個計畫行擁有有序部分以及評論、事件、版本、共用和行內資產。"}
  entities={[
    {
      id: "plans",
      name: "plans",
...
保持方塊 `type` 和 MDX `tag` 穩定。 `type` 以標準化儲存
...
const riskCardServerBlock = defineBlock<RiskCardData>({
  ...
  placement: ["block"],
  label: "Risk card",
  description: "一條 markdown 風險說明，嚴重層級可為 low、medium 或 high。",
});
...
代理，保持`label`、`description`、`placement`和`empty`精確；那些
欄位流入即時塊詞匯表。

當覆蓋現有塊時，在共用之後註冊覆蓋
資料庫註冊。 `type` 和 MDX `tag` 的最後註冊均獲勝。
...
- `app/routes/plans._index.tsx` — 計畫清單
- `app/routes/share.$token.tsx` — 公開/共用平面圖
- `app/routes/local-plans.$slug.tsx` — 本機檔案模式預覽

### 本機模式（進階、離線） {#local-mode}

對於完全離線、無帳戶的使用，您可以在本機執行計畫應用程式並將其指向本機 MDX 資料夾。對於更嚴格的無資料庫路徑，請使用 [local-files privacy mode](#local-files)，它從 MDX 資料夾讀取而不是建立本機 SQL 行。本機模式是一個單獨的進階路徑 - 不是預設的託管流程。
...
這些自動化是透過詢問計畫代理來建立的 - 無需更改程式碼。
代理使用 `action=define` 呼叫 `manage-automations`，寫入
`jobs/<name>.md`資源，事件訂閱立即開始。

#### 當有人對計畫發表評論時透過 Webhook 進行通知

詢問計畫代理：

> “當有人對計畫新增人工評論時，POST 會向我的 webhook 傳送一條訊息。”
...
傳送 - 原始 URL 永遠不會出現在代理的脈絡中。

**要專門針對 Slack：** 將 `NOTIFY_WEBHOOK` 設定為您的 Slack 傳入
網路鉤子 URL
(`https://hooks.slack.com/services/…`)。上面的自動化主體已經
產生一個有效負載 Slack 的傳入 webhook 透過 `text` 或 `blocks` 接受
欄位 - 如果您想要更豐富，請代理將內文格式化為 Slack 訊息
格式化。
...
因為自動化執行完整的代理迴圈（`mode: agentic`），所以它可以呼叫
`web-request`，傳送通知或呼叫代理有權存取的任何操作。
確切的傳遞機制取決於您擁有的通知管道
已設定 - 代理會選取最佳的可用選項。
