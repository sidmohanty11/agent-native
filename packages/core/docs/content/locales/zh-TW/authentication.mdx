...
```an-diagram title="三種方式，一次會議" summary="瀏覽器訪客、程式化 MCP 用戶端和自訂提供者都解析為下游作用域讀取的同一 AuthSession。"
{
...
```an-diagram title="遠端 MCP OAuth 握手" summary="具有 OAuth 功能的用戶端僅從 MCP URL 引導 — 質詢、發現、動態註冊，然後是 PKCE 程式碼交換。"
{
