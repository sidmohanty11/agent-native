...
```an-diagram title="每次執行都會為迴圈提供動力" summary="一次代理執行會產生跟蹤、自動評分和意見回饋掛鉤 - 所有這些都儲存在應用程式自己的 SQL 中並顯示在儀表板上。實驗將流量分配給設定變體。"
{
...
```an-diagram title="一致哈希變體分配" summary="每個使用者散列到一個穩定的變體，迴圈應用該變體的設定覆蓋，並且結果以置信區間匯總每個變體。"
{
...
所有設定都儲存在 `observability-config` 金鑰中：
...
將跟蹤傳送到 Langfuse、Datadog、Grafana 或任何 OTel 相容後端：
...
此框架發出與 OpenTelemetry GenAI 規範相容的 `gen_ai.*` 語義約定範圍。

## OpenTelemetry 跨度 {#otel}

與上面的 `exporters` 設定（將內部跟蹤傳送到 OTLP 端點）不同，代理迴圈還可以為每次執行、模型呼叫和工具呼叫發出**即時 OpenTelemetry 範圍**，因此已經執行 OTel 收集器的主機可以看到代理活動以及其分布式跟蹤的其餘部分。
...
- [**Actions**](/docs/actions) - 在跟蹤中顯示為工具呼叫的操作
