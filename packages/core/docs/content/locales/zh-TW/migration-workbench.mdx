...
description: "遷移是 Agent-Native 程式碼工作區中的內建 /migrate 目標，而不是單獨的應用程式。請參閱 Agent-Native 程式碼 UI 取得完整指南。"
---
