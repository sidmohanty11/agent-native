...
<Diagram id="doc-block-4y19o7" title={"/migrate 是一個程式碼工作階段，而不是一個單獨的應用程式"} summary={"路徑、URL 或描述進入；該執行與其他所有程式碼工作階段共用相同的儲存、指令碼和控制項，並且可以生成可移植的檔案。"}>
...
<div class="diagram-migrate">
  ...
  <div class="diagram-panel center" data-rough>
    ...
    ><small class="diagram-muted">同一儲存 · 轉錄 · 執行控制</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-box" data-rough>已遷移應用</div>
