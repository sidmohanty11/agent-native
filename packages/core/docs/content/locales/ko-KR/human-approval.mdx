  ...
  annotations={[
    {
      lines: "10",
      label: "문 전체",
      note: "깃발 하나. 진실이고 호출이 승인되지 않은 상태에서 루프는 `run` 이전에 중지됩니다. 모델은 자체적으로 부작용에 도달하지 않습니다.",
    },
    {
      lines: "11-13",
      label: "run() is untouched",
      note: "핸들러는 동일하게 유지됩니다. 승인은 `run` 내부의 어떤 것이 아니라 주변 루프에 의해 시행됩니다.",
    },
  ]}
/>
...
<Diagram id="doc-block-va61ua" title={"승인 중단"} summary={"게이트 호출은 run()이 실행되기 전에 차례를 일시 중지합니다. 승인은 통화 키를 가지고 있는 차례를 재발행합니다. 그래야만 부작용이 발생합니다."}>

```html
<div class="diagram-approve">
  <div class="diagram-box" data-rough>에이전트가 send-email 호출</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-panel warn" data-rough>
    <strong>Gate가 참, 호출은 아직 미승인</strong
  ...
  <div class="diagram-box" data-rough>
    사람이 채팅에서 승인 클릭<br /><small class="diagram-muted"
      >클라이언트가 approvedToolCalls: [approvalKey]로 턴 재발행</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
