...
description: "Agent-Native Plan skills(/visual-plan, /visual-recap, /visualize-repo)와 호스팅된 Plan MCP 커넥터를 Claude Code 또는 Codex 플러그인으로 설치하거나 범용 CLI로 설치합니다. 업데이트 작동 방식 및 제출해야 할 사항이 있는지 여부."
---

# 플러그인 및 마켓플레이스 계획

Agent-Native **Plan** 앱은 설치 가능한 하나의 번들로 제공됩니다. 한 번 설치하면 Plan slash-command skills가 추가되고 호스팅된 Plan MCP 커넥터도 연결되므로, agent는 plans를 생성하고 skills는 이를 Plan 앱에 바로 게시할 수 있습니다.

## 당신이 얻는 것 {#what-you-get}

한 번의 설치로 다음을 얻을 수 있습니다:

- **3개의 skills** — `/visual-plan`(표준 진입점), `/visual-recap`, `/visualize-repo`.
- **Plan MCP 커넥터** — `https://plan.agent-native.com`(MCP 엔드포인트 `https://plan.agent-native.com/mcp`, 서버 이름 `plan`)에서 호스팅된 앱에 대해 등록되었습니다.

<Diagram id="doc-block-u395er" title={"세 가지 경로, 하나의 번들"} summary={"범용 CLI, Claude Code 플러그인, Codex 플러그인은 모두 동일한 Plan skills와 호스팅된 Plan 커넥터를 설치합니다."}>
...
기본적으로 `/visual-plan`과 `/visual-recap`은 호스팅된 Plan 앱에 게시합니다 — MCP connector를 통해 plan을 만들고
...
<Diagram id="doc-block-1hw7qno" title={"호스팅 대 로컬 파일 모드"} summary={"기본적으로 기술은 커넥터를 통해 게시됩니다. 로컬 파일 모드는 MDX을 디스크에 쓰고 대신 localhost 브리지를 통해 미리 봅니다."}>
...
<div class="diagram-modes">
  ...
  <div class="diagram-card">
    <span class="diagram-pill warn">로컬 파일 개인정보 보호</span
    ><strong>MDX를 디스크에 쓰기</strong
    ><small class="diagram-muted"
      >plan.mdx + canvas.mdx + prototype.mdx &rarr; localhost bridge &rarr;
      hosted Plan UI reads local source. No DB 쓰기s until
...
Claude Code, Codex, Cursor, Cline, Goose, ChatGPT custom MCP apps, Claude Cowork 및 기타 MCP-compatible host에서 작동합니다. Agent-Native CLI는 Plan skills를 설치하고, 호스팅된 Plan MCP 커넥터를 등록하며, **선택한 local client(s)에 대한 인증을 같은 단계에서 실행**하므로 첫 tool call이 OAuth 벽에 막히지 않습니다:

```bash
npx @agent-native/core@latest skills add visual-plan
```

`visual-plan`과 companion skills인 `visual-recap`, `visualize-repo`를 설치한 다음 `plan` connector를 등록하고 인증을 실행합니다(hosted/account-backed sharing을 위한 OAuth prompt). 유용한 flags:
...
`/plugin install`은 Plan skills와 **URL-only** MCP config(package에 secrets 없음)를 추가합니다. `/mcp` → **Authenticate**가 OAuth handshake를 완료합니다. local-files 또는 self-hosted mode가 필요하면 대신 universal CLI route를 사용하세요.
