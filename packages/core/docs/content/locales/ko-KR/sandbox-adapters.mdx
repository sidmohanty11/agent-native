...
<Diagram id="doc-block-yzbzgh" title={"부모는 비밀을 유지합니다. 어댑터는 코드만 실행합니다."} summary={"실행 코드는 모듈을 빌드하고 루프백 브리지를 실행합니다. 어댑터는 비밀이 아닌 모듈 + 제한을 수신하고 stdout/stderr/exit을 반환합니다."}>
...
<div class="diagram-sandbox">
  <div class="diagram-box" data-rough>
    ...
    ><small class="diagram-muted"
      >모듈 빌드 · loopback 브리지 · env 정리 · 출력 형식</small
    >
  </div>
  ...
  <div class="diagram-panel center" data-rough>
    ...
    ><small class="diagram-muted">로컬 자식 · Docker · 원격 · 지속</small>
  </div>
</div>
```
...
<FileTree
  id="doc-block-12moz3v"
  title={"core의 sandbox 접점"}
