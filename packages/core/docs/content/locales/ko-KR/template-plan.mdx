...
> **대부분의 사람들은 Plan을 scaffolded app이 아니라 skill로 설치합니다.** 하나의 CLI 명령으로 coding agent에 `/visual-plan`, `/visual-recap`, `/visualize-repo` skills와 호스팅된 Plan connector를 추가합니다 — plugin 및 marketplace 경로는 [Plan plugin & marketplace](/docs/plan-plugin)를 참조하세요. Plan template fork는([For developers](#for-developers)에서 설명) self-hosting 또는 Plan 자체 위에 build하기 위한 secondary path입니다.
...
<Diagram id="doc-block-qhzm43" title={"두 개의 명령, 하나의 검토 표면"} summary={"두 명령 모두 호스팅된 Plan MCP 커넥터를 통해 동일한 주석 및 설명 화면에 게시됩니다."}>
...
<div class="diagram-plan">
  <div class="diagram-col">
    <div class="diagram-node">
      ...
      ><small class="diagram-muted">코드 전 — 아키텍처, UI, 리팩터</small>
    </div>
    <div class="diagram-node">
      <span class="diagram-pill">/visual-recap</span
      ><small class="diagram-muted">코드 후 — PR, 커밋, 브랜치, diff</small>
    </div>
  </div>
  ...
  <div class="diagram-box">
    검토 화면<br /><small class="diagram-muted"
      >다이어그램 · 와이어프레임 · 주석 코드 · 댓글</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&harr;</div>
  <div class="diagram-node">
    코딩 에이전트<br /><small class="diagram-muted">피드백 전달됨</small>
  </div>
</div>
```
...
<WireframeBlock id="doc-block-14v4b6a">
  <Screen
    surface="desktop"
    html={
...
이 명령은 `/visual-plan`, `/visual-recap`, `/visualize-repo` 세 가지 명령을 설치합니다.

MCP 커넥터 URL를 직접 허용하는 채팅 기반 호스트를 사용하는 경우
(CLI로 구성된 클라이언트 대신), 호스팅된 계획 커넥터를 다음 위치에 연결하세요.
`https://plan.agent-native.com/mcp` — 클라이언트별 설정은 [MCP Clients](/docs/mcp-clients)를 참조하세요.
...
계획이나 요약을 생성하는 것은 가벼운 템플릿 채우기가 아니라 코딩 에이전트의 LLM에 대해 완전한 에이전틱 세션을 실행하는 것이므로, 비용은 diff나 계획의 크기와 선택한 모델에 따라 달라집니다.

- 앱의 기본 채팅 모델은 GPT-5.6 Luna입니다. Anthropic에 직접 연결하면 Claude Sonnet 5가 계속 기본값으로 사용됩니다. 모델은 언제든지 **설정**에서 변경할 수 있으며, Haiku는 더 저렴한 Claude 옵션입니다.
- GitHub Action의 자동화된 PR 요약은 인터랙티브 앱과 별도로 실행되며 자체 모델 및 비용 조정 옵션을 갖습니다. `VISUAL_RECAP_MODEL`, `VISUAL_RECAP_AGENT`, `VISUAL_RECAP_REASONING`에 대해서는 [PR Visual Recap](/docs/pr-visual-recap)를 참조하세요.
...
호스팅된 Plans 커넥터는 `https://plan.agent-native.com/mcp`에 있습니다.
...
## 저장소 시각 문서 {#visualize-repo}

일회성 plan이나 recap이 아니라 codebase를 위한 지속적인 시각 문서 레이어가 필요할 때 `/visualize-repo`를 사용합니다. 로컬 Plan MDX workspace를 만들거나 열고, `agent-native.json`에 `apps.visualize-repo` local-files 섹션을 추가하며, 같은 localhost bridge를 통해 docs를 제공합니다.

```bash
npx @agent-native/core@latest visualize-repo --open
```

`agent-native.json`이 없으면 이 명령은 파일을 bootstrap하고 repo 구조에서 `actions/`, `app/components/`, `app/pages/`, `server/db/schema.ts`, `src/`, `packages/`, `templates/`, `docs/`, `content/` 같은 작은 starter set을 추천합니다. 특정 source anchor를 target할 수도 있습니다.
...
Starter folder는 기본적으로 `.agent-native/visual-docs/repo-overview`에 있습니다. Comments는 `comments.json`에 local로 남기 때문에 reviewer가 coding agent에게 보낼 섹션을 표시할 수 있고, agent는 같은 change에서 code와 MDX를 함께 업데이트할 수 있습니다. Local lint에는 `visualize-repo check`를 실행하고, Plan renderer schema로 MDX를 validate하려면 `visualize-repo verify`를 실행하세요.
...
- "공개 API actions와 schema부터 시작해서 `/visualize-repo`를 실행하세요."
...
<DataModel
  id="doc-block-1javm2u"
  title={"계획 데이터 모델"}
  summary={
    "하나의 계획 행은 주문된 섹션과 설명, 이벤트, 버전, 공유 및 인라인 자산을 소유합니다."
  }
  entities={[
    {
      id: "plans",
