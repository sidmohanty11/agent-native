...
- 같은 스레드에 나중에 남긴 일반 답글은 **해당 스레드에 아직 실행 중인 작업이 있을 때만** 에이전트에 도달합니다. 실행이 끝났다면 후속 메시지는 무시되므로, 에이전트가 계속 듣고 있다고 가정하지 말고 다시 @mention하세요.
...
| 한 번은 됐는데 이후 스레드 답글이 무시됨                  | 정상 동작입니다. 멘션 없는 스레드 답글은 그 스레드에 활성 작업이 있는 동안에만 에이전트에 도달합니다. 다시 @mention하세요.   |
