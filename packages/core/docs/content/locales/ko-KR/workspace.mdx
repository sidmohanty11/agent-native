...
<Diagram id="doc-block-19r4wnf" title={"Claude-Code 작업 공간이지만 SQL에 저장됨"} summary={"동일한 사용자 정의 계층(지침, 기술, 메모리, 에이전트, 작업, MCP)은 모든 파일이 공유 다중 테넌트 데이터베이스의 행이라는 점을 제외합니다."}>
...
<div class="ws-map">
  ...
  <div class="diagram-card an">
    <span class="diagram-pill accent">Agent-native 워크스페이스</span
...
<DataModel
  ...
  title={"작업공간 자원 모델"}
  summary={
    "하나의 리소스 형태는 모든 작업 공간 파일을 백업합니다. 런타임은 이를 경로 및 범위별로 키를 지정하고 읽기 시 유효 값을 확인합니다."
  }
  entities={[
    {
      id: "resource",
