...
<Diagram id="doc-block-11e3jlp" title={"클라이언트 방향: 다양한 소스, 하나의 도구 레지스트리"} summary={"구성 파일, 환경 및 런타임 UI는 모두 MCP 관리자로 병합됩니다. 해당 도구는 접두사가 붙어 표시되며 앱 작업과 함께 도구 검색이 가능합니다. 이는 서버 방향의 미러입니다."}>
...
<div class="mcp-merge">
  <div class="diagram-col sources">
    ...
    <div class="diagram-box" data-rough>
      앱 루트 <code>mcp.config.json</code><br /><small class="diagram-muted"
        >앱별 재정의</small
      >
    </div>
    <div class="diagram-box" data-rough>
    ...
    <div class="diagram-box" data-rough>
      설정 UI를 통한 원격<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">MCP 관리자</span
    ><small class="diagram-muted">merge &middot; hot-reload</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col out">
    <div class="diagram-node">
      에이전트 도구 registry<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-1mpqn6q"
  title={"mcp.config.json, 주석 처리됨"}
  ...
  annotations={[
    {
      lines: "3",
      label: "서버ID",
      note: "키는 도구 접두사가 됩니다. 이 서버의 도구는 에이전트 레지스트리에 `mcp__claude-in-chrome__*`으로 표시되므로 템플릿 작업과 충돌할 수 없습니다.",
    },
    {
      lines: "4-6",
      label: "stdio launcher",
      note: "`command` + `args`는 로컬 바이너리를 생성합니다. Stdio 서버는**로컬 개발**용으로 설계되었으며 Edge 런타임에서는 작동하지 않습니다.",
    },
    {
      lines: "6",
      label: "프로세스 환경",
...
프리셋 ID는 `context7`, `sentry`, `notion`, `semgrep`, `linear`,
`atlassian`, `supabase`, `neon`, `stripe`, `cloudflare`, `gitlab`, `figma`,
`vercel`, `github`, `slack`, `asana`, `hubspot`, `intercom`, `monday`, `webflow`, `paypal`, `box`, `netlify`, `zapier`입니다.

디렉터리는 일반 OAuth 흐름을 지원하는 엔드포인트와 공급자가 승인한 클라이언트
또는 사전 등록된 OAuth 앱이 필요한 공급자를 구분합니다. 후자의 항목도 검색을
위해 표시하고 공식 설정 안내로 연결하지만, 지원되지 않는 경우 오해를 부르는
원클릭 연결 버튼은 표시하지 않습니다. 샌드박스 확장은 호스트 액션을 통해
`agentNative.mcp.listTools`, `agentNative.mcp.callTool`,
`agentNative.providerApi.catalog(...)`, `agentNative.providerApi.docs(...)`를
사용해 연결된 데이터를 검색할 수 있으며, 연결 URL이나 액세스 토큰, 리프레시
토큰은 절대 받지 않습니다. 원시 `provider-api-request`는 의도적으로 호스트와
에이전트에서만 사용할 수 있습니다. 임의의 메서드/경로 요청을 공유 확장에
노출하면 해당 확장이 인증된 쓰기 프록시가 될 수 있기 때문입니다. 확장은
대신 범위가 제한된 공급자 작업을 위해 `agentNative.mcp.callTool(...)`이나
목적에 맞게 만들어진 앱 액션을 사용해야 합니다.

로컬 파일 확장은 `extension.json`에서 `list-mcp-tools`, `call-mcp-tool` 및
사용하는 지원되는 공급자 액션을 명시적으로 허용해야 합니다. 검색 도우미는
`agentNative.connectors.mcp` 및 `agentNative.connectors.providerApi`에서도
사용할 수 있습니다.

### Atlassian / Jira

Atlassian 프리셋은 Atlassian Rovo MCP의 OAuth 2.1 엔드포인트인
`https://mcp.atlassian.com/v1/mcp/authv2`를 사용하며 Jira 및 Confluence
도구를 제공합니다. Jira 티켓을 만들려면 Jira 쓰기를 허용하는 Atlassian
권한이 필요합니다. 사용자는 계속해서 자신의 Atlassian 계정에 허용된
프로젝트와 콘텐츠만 볼 수 있습니다.

Atlassian 조직 관리자는 OAuth 클라이언트의 연결 허용 여부를 제어할 수
있습니다. 연결하기 전에 관리자가 앱의 실제 HTTPS 오리진을
**Atlassian Administration → Rovo → Rovo MCP server → Domains**의 신뢰할 수
있는 도메인 목록에 추가하고, **Permissions**에서 필요한 **Read**,
**Write**, **Search** 권한을 활성화해야 할 수 있습니다. 도메인은
Agent-Native 앱이 실행되는 호스트여야 합니다. 실제 오리진이 아니라면 다른
배포에 `https://builder.io/**`를 복사하지 마세요. Atlassian의 [Rovo MCP 설정
가이드](https://developer.atlassian.com/cloud/rovo-mcp/guides/getting-started/)와
[관리자 제어](https://support.atlassian.com/security-and-access-policies/docs/control-atlassian-rovo-mcp-server-settings/)를 참조하세요.

카탈로그는 엔드포인트와 설정 안내를 제공합니다. 일반 설정 양식은 엔드포인트와
선택적 암호화 헤더를 저장하지만, Atlassian의 대화형 OAuth 동의 흐름을
대체하지 않습니다. Atlassian에서 대화형 OAuth를 요구하는 경우 OAuth를 지원하는
MCP 클라이언트 또는 런타임을 사용하세요. 공급자 또는 런타임에서 정적 Bearer
토큰을 지원하는 경우 승인된 비밀 관리 절차를 따르고 프롬프트나 커밋된 설정에
토큰을 붙여 넣지 마세요.
...
HTTPS URL는 모든 곳에서 허용됩니다. 일반 `http://`는 개발 중에 `localhost`에만 허용됩니다. 선택적 인증은 모든 요청에서 `Authorization: Bearer …`를 통해 전송되는 Bearer 토큰으로 들어갑니다. Atlassian Rovo MCP를 포함한 OAuth 전용 공급자는 정적 헤더 대신 공급자의 OAuth 흐름을 사용해야 합니다.
...
<Diagram id="doc-block-12q3mwq" title={"허브 모델: 하나의 앱이 조직 범위 MCP 서버를 제공합니다."} summary={"Dispatch는 조직 범위 MCP 서버를 보유합니다. 소비자 앱은 이를 mcp__hub_<orgId>_<name>__*으로 끌어오고 병합합니다. 조직 범위 행만 공유되며 개인 자격 증명은 그대로 유지됩니다."}>

```html
<div class="mcp-hub">
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Dispatch hub</span
    ><small class="diagram-muted">조직 범위 MCP 서버</small
  ...
  <div class="diagram-col consumers">
    ...
    <div class="diagram-box" data-rough>
      Clips<br /><small class="diagram-muted">약 60초마다 pull + merge</small>
    </div>
  </div>
</div>
...
<Endpoint id="doc-block-bvepqr" method="GET" path="/_agent-native/mcp/status" summary={"도구 및 온보딩을 위한 MCP 클라이언트 상태"} responses={[
  {
    "status": "200",
...
- 잘못 구성된 `command` → 서버를 건너뛰고 해당 오류가 `errors.<server-id>` 아래의 `/_agent-native/mcp/status`에 나타나고 다른 모든 서버는 계속 작동합니다.
