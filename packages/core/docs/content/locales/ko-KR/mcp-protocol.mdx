...
- **자동 마운트** — 모든 앱에 `/mcp`가 무료로 제공되며 설정이 필요하지 않습니다.
...
<Endpoint id="doc-block-6peopk" title={"MCP 엔드포인트"} summary={"모든 에이전트 기반 앱이 노출하는 자동 탑재 스트리밍 가능 HTTP 엔드포인트입니다."} method="POST" path="/mcp" summary={"MCP 스트리밍 가능한 HTTP 엔드포인트"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
호스팅된 MCP 요청은 제한된 인라인 대기 시간을 사용하므로, 긴 에이전트 루프 때문에
클라이언트나 게이트웨이가 시간 초과될 때까지 하나의 `tools/call`이 열려 있지
않습니다. 선택적 `async` 인수를 사용하면 내구성 있는 작업을 시작하고 즉시
반환됩니다. 이 인수가 없으면 서버는 빠른 완료를 짧게 기다린 다음, 작업이 더
남아 있으면 작업 핸들을 반환합니다. 호스팅된 대기 시간은 최대 25초로
제한됩니다.
...
에이전트는 대화형 채팅과 동일한 루프를 실행합니다. 즉, 여러 도구를 호출하고 상황에 대해 추론하고 사려 깊은 응답을 생성할 수 있습니다. 완료되지 않은 작업은 `taskId`, `status`, `pollAfterMs`를 포함하는 JSON과 함께 다음을 반환합니다.
...
작업이 최종 상태에 도달할 때까지 해당 인수로 `ask_app_status`를 호출하세요.
...
| `/mcp/oauth/register`                     | 동적 공개 클라이언트 등록        |
| `/mcp/oauth/authorize`                    | 브라우저 승인 + 동의             |
| `/mcp/oauth/token`                        | 인증 코드 및 새로 고침 토큰 부여 |
