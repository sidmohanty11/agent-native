...
<Diagram id="doc-block-g00ug3" title={"하나의 스키마, 많은 백엔드"} summary={"앱 코드는 프레임워크의 방언에 구애받지 않는 도우미를 사용합니다. 방언은 런타임 시 DATABASE_URL에서 자동 감지됩니다. unset은 로컬 SQLite 파일을 의미합니다."}>
  ...
  annotations={[
    {
      lines: "6-7",
      label: "첨가제만",
      note: "`ADD COLUMN IF NOT EXISTS`은 다시 실행해도 안전하며 데이터를 삭제하지 않습니다. 이름 바꾸기는 Drizzle에 drop+create와 유사하므로 대신 추가 후 마이그레이션하세요.",
    },
    {
      lines: "13-16",
      label: "방언 게이팅",
    ...
    },
    {
      lines: "19",
      label: "앱별 버전 표",
      note: "각 앱은 자체 적용된 버전을 추적하므로 마이그레이션은 다시 시작 및 인스턴스 전반에 걸쳐 멱등성을 갖습니다.",
    },
  ]}
/>
