...
| `ask_app({ app, message, async?, maxWaitMs? })`    | 에이전트 루프 | 자연어 작업을 해당 앱의 인앱 에이전트로 라우팅합니다. 오래 걸리는 작업은 `ask_app_status` 폴링을 위한 내구성 있는 `taskId`를 반환합니다 |
...
작업의 접수를 빠르게 확인한 뒤, 인라인 대기 시간이 만료되면 내구성 있는 `taskId`를
반환합니다. 반환된 `app`과 `taskId`를 사용해 `ask_app_status`로 폴링하세요. 결과에는
