...
- **MCP 커넥터 1개를 노출합니다.** 추가
  Claude, ChatGPT의 `https://dispatch.agent-native.com/mcp`,
...
- **MCP 허브 모드.** 디스패치는 여전히 작업 영역의 [MCP hub](/docs/mcp-clients#hub) 역할을 할 수 있으므로 작업 영역의 다른 모든 앱은 동일한 조직 범위 MCP 서버 목록을 가져옵니다. 이와 별도로 Dispatch의 자체 `/mcp` 엔드포인트는 Claude, ChatGPT 및 여러 작업 공간 앱에 연결해야 하는 기타 호스트에 권장되는 외부 MCP 커넥터입니다.
