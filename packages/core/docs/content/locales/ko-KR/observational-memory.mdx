...
<Diagram id="doc-block-n1ot61" title={"3단계, 최근 단계로 증류됨"} summary={"오래된 접두사는 날짜가 지정된 관찰과 긴 호 반사로 접혀집니다. 가장 최근 턴만 그대로 유지됩니다."}>

```html
<div class="om">
  <div class="diagram-card">
    <span class="diagram-pill">회고</span
    ><small class="diagram-muted">관찰 로그에서 압축한 장기 요약</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&uarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill accent">관찰</span
    ><small class="diagram-muted"
      >원본 메시지 구간을 접은 조밀한 날짜별 항목</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&uarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill ok">최근 원본 메시지</span
    ><small class="diagram-muted"
      >last N turns, kept <strong>verbatim</strong> — 절대 접지 않음</small
    >
  </div>
</div>
```
...
<Diagram id="doc-block-1hw22ay" title={"깔끔한 회전 후 두 번의 최선의 패스"} summary={"각 패스는 임계값 이하로 작동하지 않으므로 매 턴마다 압축기를 실행하는 것이 저렴합니다. 실패는 무시되며 대기 시간을 추가하지 않습니다."}>

```html
<div class="om-pass">
  <div class="diagram-node">
    깨끗한 턴 종료<br /><small class="diagram-muted">fire-and-forget</small>
  </div>
