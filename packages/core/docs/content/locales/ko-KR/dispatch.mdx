...
<Endpoint id="doc-block-j0rg0r" method="POST" path="/_agent-native/mcp" summary={"통합 MCP 게이트웨이 엔드포인트"} auth="Standard remote MCP OAuth, handled by the framework. The granted-app set scopes which workspace apps the connector can reach." responses={[
  {
    "status": "200",
