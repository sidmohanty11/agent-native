...
<Diagram id="doc-block-6q3x33" title={"사다리 원리"} summary={"대부분의 팀은 렁 1 또는 2에서 멈춥니다. 에이전트 네이티브는 렁 3입니다. 즉, 하나의 공유 작업 표면에 대한 실제 앱이자 실제 에이전트입니다."}>

```html
<div class="diagram-ladder">
  <div class="diagram-card rung rung-3">
    <span class="diagram-pill accent">단계 3 · agent-native</span
    ><strong>에이전트 + UI가 동등한 파트너</strong
  ...
  <div class="diagram-card rung rung-2">
    <span class="diagram-pill">단계 2</span><strong>도구가 있는 채팅</strong
    ><small class="diagram-muted"
      >에이전트는 동작할 수 있지만 여전히 채팅 창일 뿐입니다. 대시보드, 목록,
      단축키는 없습니다.</small
    >
  </div>
  <div class="diagram-card rung rung-1">
    <span class="diagram-pill warn">단계 1</span><strong>단일 LLM 호출</strong
    ><small class="diagram-muted"
...
<Diagram id="doc-block-jh2i6u" title={"하나의 시스템, 두 가지 방식"} summary={"에이전트와 UI는 동일한 작업과 동일한 데이터베이스에 기록합니다. 한 사람이 무엇을 하든 다른 사람이 본다."}>

```html
<div class="diagram-parity">
  <div class="diagram-col">
    <div class="diagram-node">
      Human<br /><small class="diagram-muted">클릭, 폼, 단축키</small>
    </div>
    <div class="diagram-node">
      Agent<br /><small class="diagram-muted">자연어 · A2A · Slack</small>
    </div>
  </div>
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">SQL 데이터베이스</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-box">UI가 실시간 업데이트</div>
</div>
```
...
에이전트 네이티브는 앱을 종료하지 않고도 모든 사용자에게 동일한 사용자 정의 레이어를 제공합니다. 각 앱에는 사용자(또는 팀 구성원 누구나)가 다음을 수행할 수 있는 개인 **에이전트 리소스**가 제공됩니다.
...
트위스트: 파일 시스템이 아닌 데이터베이스에 모두 저장됩니다. 가동할 개발 환경도 없고 사용자당 컨테이너도 없습니다. 모든 사용자는 개인 메모리, 개인 연결, 개인 skills 등 자신만의 전체 에이전트 리소스를 본질적으로 무료로 얻습니다. 왜냐하면 모든 것이 테이블의 행이기 때문입니다. 이것이 바로 실제 멀티 테넌트 SaaS 제품 내에서 Claude 코드 수준의 유연성을 실현할 수 있는 이유입니다.

전체 개념은 [에이전트 리소스](/docs/agent-resources)를 참조하세요.
...
<AnnotatedCode
  id="doc-block-1t9ytb6"
  title={"한 번 정의된 하나의 작업"}
  ...
  annotations={[
    {
      lines: "5",
      label: "도구 표면",
      note: "`description`는 이를 도구로 호출할 시기를 결정하기 위해 에이전트가 읽는 것입니다.",
    },
    {
      lines: "6",
      label: "타입 계약",
      note: "하나의 zod `schema`가 에이전트, UI, HTTP, MCP, A2A 등 **모든** 표면의 입력을 검증합니다.",
    },
    {
      lines: "7-10",
      label: "하나의 구현",
      note: "`run` 본문은 단일 정보 소스입니다. UI 버튼과 에이전트 도구 모두 이를 정확하게 실행합니다.",
    },
  ]}
/>
...
- [**에이전트 리소스**](/docs/agent-resources) — 파일이 아닌 SQL가 지원하는 사용자별 사용자 정의 레이어(skills, 메모리, 지침, MCP)
