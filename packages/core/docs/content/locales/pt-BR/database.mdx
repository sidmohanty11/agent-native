...
<Diagram id="doc-block-16ajvk8" title="Um esquema, muitos back-ends" summary={"O código do aplicativo usa os auxiliares independentes de dialeto da estrutura. O dialeto é detectado automaticamente em DATABASE_URL em tempo de execução; unset significa um arquivo SQLite local."}>
...
<DataModel
  id="doc-block-19her11"
  title="A tabela de tarefas"
  summary={
    "Definida uma vez com os auxiliares do framework; o dialeto é escolhido em tempo de execução a partir de DATABASE_URL."
  }
  entities={[
    {
      id: "tasks",
...
<AnnotatedCode
  id="doc-block-vhf88u"
  title={"Um plugin de migração aditivo"}
  ...
  annotations={[
    {
      lines: "6-7",
      label: "Somente aditivo",
      note: "`ADD COLUMN IF NOT EXISTS` é seguro para ser executado novamente e nunca descarta dados. As renomeações parecem drop+create para Drizzle, então adicione e migre.",
    },
    {
      lines: "13-16",
      label: "Controle de dialeto",
    ...
    },
    {
      ...
      note: "Cada aplicativo rastreia suas próprias versões aplicadas para que as migrações sejam idempotentes entre reinicializações e instâncias.",
    },
  ]}
/>
