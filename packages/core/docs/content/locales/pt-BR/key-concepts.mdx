  ...
  <div class="diagram-row">
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >lê e grava dados, executa ações, modifica código</small
      >
    </div>
    <div class="diagram-card">
...
(`databaseTools: "read"`). Os agentes inspecionam dados do app com `db-schema` /
`db-query` e escrevem por meio de actions tipadas do app. Defina
`databaseTools: "write"` (ou `true`) apenas para superfícies de manutenção
intencionais que também devam expor `db-exec` / `db-patch` com escopo; use
`databaseTools: "off"` / `false` para exigir actions tipadas para todo acesso a dados.
