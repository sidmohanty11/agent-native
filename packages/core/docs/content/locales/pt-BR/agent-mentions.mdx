...
<Diagram id="doc-block-ta9yf8" title={"Onde uma @-menção direciona"} summary={"O servidor divide cada menção por tipo: agentes personalizados são executados localmente, agentes conectados passam por A2A — ambas as respostas voltam ao contexto do agente principal."}>
...
<div class="diagram-mention">
  ...
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Servidor resolve</span
...
Consulte [Recursos do Agente — Custom Agents](/docs/agent-resources#custom-agents) para obter o formato completo (incluindo `tools`, `delegate-default` e substituições de modelo).
...
<AnnotatedCode
  id="doc-block-jfl92k"
  title="Um MentionProvider personalizado"
