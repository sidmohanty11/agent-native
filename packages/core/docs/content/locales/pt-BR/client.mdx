...
<Diagram id="doc-block-1ojowdw" title="O loop de dados do navegador" summary={"Ganchos leem e escrevem por meio de ações; useDbSync observa o banco de dados para que as gravações do agente e em segundo plano busquem novamente os mesmos caches automaticamente."}>
...
<Diagram id="doc-block-7z6lct" title="Dois caminhos para uma nova busca" summary={"Uma mutação local invalida instantaneamente seus próprios caches; uma gravação remota chega a esta guia por meio de SSE ou o tick de votação como substituto."}>
