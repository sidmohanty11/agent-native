...
<Diagram id="doc-block-1l7jkfd" title={"Uma configuração Claude-Code, mas armazenada como recursos do agente em SQL"} summary={"A mesma camada de personalização — instruções, habilidades, memória, agentes, trabalhos, MCP — exceto que cada arquivo é uma linha em um banco de dados multilocatário compartilhado."}>
