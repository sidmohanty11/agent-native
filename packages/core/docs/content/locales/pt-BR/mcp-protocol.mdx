...
- **Montado automaticamente** — todos os aplicativos recebem `/mcp` gratuitamente, sem necessidade de configuração
...
| **Ponto final**               | `/mcp`                                                                 | `/_agent-native/a2a`                               |
...
As solicitações MCP hospedadas usam uma espera embutida limitada para que um loop de agente longo não mantenha um `tools/call` aberto até um timeout do cliente ou do gateway. O argumento opcional `async` inicia a tarefa durável e retorna imediatamente; sem ele, o servidor aguarda brevemente por uma conclusão rápida e depois retorna um identificador de tarefa quando ainda resta trabalho. A espera hospedada tem um limite de 25 segundos.
...
O agente executa o mesmo loop que o bate-papo interativo: ele pode chamar diversas ferramentas, raciocinar sobre o contexto e produzir uma resposta cuidadosa. Uma tarefa que ainda não terminou retorna um JSON contendo `taskId`, `status`, `pollAfterMs` e:
...
Chame `ask_app_status` com esses argumentos até que a tarefa alcance um estado
terminal. O `ask_app` de propriedade do aplicativo no Dispatch usa o mesmo
...
| `/mcp/oauth/register`                     | Registro dinâmico de cliente público                       |
| `/mcp/oauth/authorize`                    | Autorização + consentimento do navegador                   |
| `/mcp/oauth/token`                        | Concessões de código de autorização e token de atualização |
