...
<Endpoint id="doc-block-lgsyj9" method="POST" path="/_agent-native/mcp" summary="Ponto de extremidade do gateway MCP unificado" auth="Standard remote MCP OAuth, handled by the framework. The granted-app set scopes which workspace apps the connector can reach." responses={[
  {
    "status": "200",
