...
<Diagram id="doc-block-mo3k6d" title={"O Princípio da Escada"} summary={"A maioria das equipes para na linha 1 ou 2. O agente nativo é a linha 3 – um aplicativo real e um agente real em uma superfície de ação compartilhada."}>
...
<div class="diagram-ladder">
  ...
  <div class="diagram-card rung rung-2">
    ...
    ><small class="diagram-muted"
      >O agente pode agir, mas ainda é só uma janela de chat. Sem dashboards,
      listas ou atalhos.</small
    >
  </div>
...
<Diagram id="doc-block-1vadipv" title="Um sistema, duas maneiras de entrar" summary={"O agente e a UI gravam nas mesmas ações e no mesmo banco de dados. O que quer que um faça, o outro vê."}>
...
<div class="diagram-parity">
  <div class="diagram-col">
    <div class="diagram-node">
      Human<br /><small class="diagram-muted"
        >cliques, formulários, atalhos</small
      >
    </div>
    <div class="diagram-node">
      Agent<br /><small class="diagram-muted"
        >linguagem natural · A2A · Slack</small
      >
    </div>
  </div>
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">banco de dados SQL</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-box">A UI atualiza ao vivo</div>
</div>
```
...
O Agent-native oferece a todos os usuários a mesma camada de personalização, sem nunca sair do aplicativo. Cada aplicativo vem com **recursos do agente** pessoais que você (ou qualquer pessoa da sua equipe) pode usar para:
  ...
  annotations={[
    {
      ...
      note: "O `description` é o que o agente lê para decidir quando chamar isso como ferramenta.",
    },
    {
      lines: "6",
      label: "Contrato tipado",
      note: "Um zod `schema` valida a entrada de **todas** as superfícies — agente, UI, HTTP, MCP e A2A.",
    },
    {
      lines: "7-10",
      label: "Uma implementação",
      note: "O corpo `run` é a única fonte da verdade. O botão UI e a ferramenta do agente executam exatamente isso.",
    },
  ]}
/>
...
- [**Recursos do Agente**](/docs/agent-resources) — a camada de personalização por usuário (skills, memória, instruções, MCP) apoiada por SQL, não por arquivos
