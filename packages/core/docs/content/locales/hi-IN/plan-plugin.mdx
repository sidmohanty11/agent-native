...
description: "Agent-Native Plan skills (/visual-plan, /visual-recap, /visualize-repo) और hosted Plan MCP connector को Claude Code या Codex प्लगइन के रूप में, या universal CLI के साथ install करें। Updates कैसे काम करते हैं और क्या आपको कुछ submit करना है।"
---

# प्लान प्लगइन और मार्केटप्लेस

Agent-Native **Plan** app एक installable bundle के रूप में आता है। एक ही install Plan slash-command skills जोड़ता है **और** hosted Plan MCP connector को wire करता है, ताकि agent plans generate कर सके और skills उन्हें सीधे Plan app में publish कर सकें।

## आपको क्या मिलता है {#what-you-get}

एक इंस्टॉल आपको देता है:

- **तीन skills** — `/visual-plan` (canonical entry point), `/visual-recap`, और `/visualize-repo`।
- **प्लान MCP कनेक्टर** - `https://plan.agent-native.com` (MCP एंडपॉइंट `https://plan.agent-native.com/mcp`, सर्वर नाम `plan`) पर होस्ट किए गए ऐप के खिलाफ पंजीकृत है।

<Diagram id="doc-block-gdfyxv" title={"तीन मार्ग, एक बंडल"} summary={"Universal CLI, Claude Code प्लगइन, और Codex प्लगइन सभी वही Plan skills और hosted Plan connector install करते हैं।"}>
...
डिफ़ॉल्ट रूप से, `/visual-plan` और `/visual-recap` hosted Plan app में publish करते हैं — वे plan बनाते हैं
...
<Diagram id="doc-block-ah49lf" title={"होस्टेड बनाम स्थानीय-फ़ाइलें मोड"} summary={"डिफ़ॉल्ट रूप से कौशल कनेक्टर के माध्यम से प्रकाशित होते हैं; लोकल-फ़ाइलें मोड डिस्क पर MDX लिखता है और इसके बजाय लोकलहोस्ट ब्रिज के माध्यम से पूर्वावलोकन करता है।"}>

```html
<div class="diagram-modes">
  <div class="diagram-card">
    <span class="diagram-pill accent">डिफॉल्ट · होस्टेड</span
  ...
  <div class="diagram-card">
    <span class="diagram-pill warn">स्थानीय फाइलों की गोपनीयता</span
    ><strong>MDX को डिस्क पर लिखें</strong
    ><small class="diagram-muted"
      >plan.mdx + canvas.mdx + prototype.mdx &rarr; localhost bridge &rarr;
      hosted Plan UI reads local source. No DB लेखनs until
...
किसी भी host के साथ काम करता है — Claude Code, Codex, Cursor, Cline, Goose, ChatGPT custom MCP apps, Claude Cowork, और कोई भी MCP-compatible host। Agent-Native CLI Plan skills install करता है, hosted Plan MCP connector register करता है, **और उसी step में चुने गए local client(s) के लिए auth चलाता है**, ताकि आपकी पहली tool call OAuth wall से न टकराए:

```bash
npx @agent-native/core@latest skills add visual-plan
```

यह `visual-plan` के साथ companion skills `visual-recap` और `visualize-repo` install करता है, फिर `plan` connector register करता है, फिर auth चलाता है (hosted/account-backed sharing के लिए OAuth prompt)। उपयोगी flags:
...
`/plugin install` Plan skills और **URL-only** MCP config जोड़ता है (package में कोई secrets नहीं); `/mcp` → **Authenticate** OAuth handshake पूरा करता है। जब local-files या self-hosted mode चाहिए, तो universal CLI route का उपयोग करें।
...
इंस्टॉल करने के बाद, **एक नया Codex थ्रेड शुरू करें** ताकि skills और MCP टूल सत्र में लोड हो जाएं। प्लगइन एक URL-केवल कनेक्टर (`[mcp_servers.plan]` → `https://plan.agent-native.com/mcp`) शिप करता है; `codex mcp login plan` OAuth प्रवाह चलाता है। उपरोक्त सार्वभौमिक CLI मार्ग Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`) के लिए भी काम करता है यदि आप एक कमांड पसंद करते हैं जो एक साथ स्थापित और प्रमाणित करता है, या जब आप स्थानीय-फ़ाइलें या स्वयं-होस्टेड मोड चाहते हैं।
