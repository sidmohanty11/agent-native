...
> **ज़्यादातर लोग Plan को scaffolded app के बजाय skill के रूप में install करते हैं।** एक CLI command आपके coding agent में `/visual-plan`, `/visual-recap`, और `/visualize-repo` skills के साथ hosted Plan connector जोड़ता है — plugin और marketplace routes के लिए [Plan plugin & marketplace](/docs/plan-plugin) देखें। Plan template को fork करना ([For developers](#for-developers) में शामिल) self-hosting या Plan पर build करने का secondary path है।
...
<Diagram id="doc-block-52icv8" title={"दो आदेश, एक समीक्षा सतह"} summary={"दोनों कमांड होस्ट किए गए प्लान MCP कनेक्टर के माध्यम से एक ही एनोटेट-और-टिप्पणी सतह पर प्रकाशित होते हैं।"}>
  ...
  <div class="diagram-col">
    <div class="diagram-node">
      ...
      ><small class="diagram-muted"
        >कोड से पहले — आर्किटेक्चर, UI, रिफैक्टर</small
      >
    </div>
    <div class="diagram-node">
...
<WireframeBlock id="doc-block-cqiw2w">
  <Screen
    surface="desktop"
    html={
...
कमांड तीन कमांड इंस्टॉल करता है: `/visual-plan`, `/visual-recap`, और `/visualize-repo`।
...
`https://plan.agent-native.com/mcp` - क्लाइंट-विशिष्ट सेटअप के लिए [MCP Clients](/docs/mcp-clients) देखें।
...
कोई योजना या रिकैप बनाना आपके कोडिंग एजेंट के LLM पर एक पूर्ण एजेंटिक सत्र चलाता है, न कि कोई हल्का टेम्प्लेट फिल, इसलिए लागत diff या योजना के आकार और आपके द्वारा चुने गए मॉडल के अनुसार बढ़ती है।

- ऐप का डिफ़ॉल्ट चैट मॉडल GPT-5.6 Luna है। सीधे Anthropic कनेक्शन में Claude Sonnet 5 डिफ़ॉल्ट बना रहता है। मॉडल को कभी भी **सेटिंग्स** में बदलें; Haiku सस्ता Claude विकल्प है।
- GitHub Action से स्वचालित PR रिकैप इंटरैक्टिव ऐप से अलग चलते हैं और इनके अपने मॉडल और लागत नियंत्रण होते हैं — `VISUAL_RECAP_MODEL`, `VISUAL_RECAP_AGENT`, और `VISUAL_RECAP_REASONING` के लिए [PR Visual Recap](/docs/pr-visual-recap) देखें।
...
होस्ट किया गया प्लान कनेक्टर `https://plan.agent-native.com/mcp` पर रहता है।
...
## रिपॉज़िटरी के विज़ुअल दस्तावेज़ {#visualize-repo}

`/visualize-repo` का उपयोग तब करें जब आपको एक बार के प्लान या रिकैप के बजाय कोडबेस के लिए टिकाऊ विज़ुअल दस्तावेज़ीकरण परत चाहिए। यह स्थानीय Plan MDX workspace बनाता या खोलता है, `agent-native.json` को `apps.visualize-repo` local-files सेक्शन से अपडेट करता है, और उसी localhost bridge से docs दिखाता है:

```bash
npx @agent-native/core@latest visualize-repo --open
```

यदि `agent-native.json` मौजूद नहीं है, तो कमांड उसे bootstrap करता है और repo structure से छोटा starter set सुझाता है, जैसे `actions/`, `app/components/`, `app/pages/`, `server/db/schema.ts`, `src/`, `packages/`, `templates/`, `docs/`, या `content/`। आप specific source anchors भी target कर सकते हैं:
...
Starter folder डिफ़ॉल्ट रूप से `.agent-native/visual-docs/repo-overview` में रहता है। Comments `comments.json` में local रहते हैं, इसलिए reviewers sections को coding agent के लिए mark कर सकते हैं और agent उसी change में code और MDX दोनों update कर सकता है। Local lint के लिए `visualize-repo check` चलाएं, और जब Plan renderer schema से MDX validate कराना हो तो `visualize-repo verify` चलाएं।
...
- "हमारे सार्वजनिक API actions और schema से शुरू करते हुए `/visualize-repo` चलाएं।"
...
<DataModel
  id="doc-block-16ye6ti"
  title={"योजना डेटा मॉडल"}
  summary={
    "एक योजना पंक्ति में आदेशित अनुभागों के अलावा टिप्पणियाँ, ईवेंट, संस्करण, शेयर और इनलाइन संपत्तियां होती हैं।"
  }
  entities={[
    {
      id: "plans",
