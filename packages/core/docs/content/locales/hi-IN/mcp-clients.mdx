...
<Diagram id="doc-block-zyq79s" title={"ग्राहक दिशा: कई स्रोत, एक टूल रजिस्ट्री"} summary={"कॉन्फ़िग फ़ाइलें, env, और रनटाइम UI सभी MCP प्रबंधक में विलीन हो जाते हैं; इसके उपकरण आपके ऐप के कार्यों के साथ-साथ उपसर्ग और उपकरण-खोज योग्य दिखाई देते हैं। यह सर्वर दिशा का दर्पण है."}>
...
<div class="mcp-merge">
  <div class="diagram-col sources">
    ...
    <div class="diagram-box" data-rough>
      ऐप रूट <code>mcp.config.json</code><br /><small class="diagram-muted"
        >प्रति-ऐप override</small
      >
    </div>
    <div class="diagram-box" data-rough>
    ...
    <div class="diagram-box" data-rough>
      सेटिंग्स UI से रिमोट<br /><small class="diagram-muted"
...
<AnnotatedCode
  id="doc-block-1c9ae56"
  title={"mcp.config.json, एनोटेट"}
  ...
  annotations={[
    {
      lines: "3",
      label: "सर्वर आईडी",
      note: "कुंजी उपकरण उपसर्ग बन जाती है: इस सर्वर के उपकरण एजेंट की रजिस्ट्री में `mcp__claude-in-chrome__*` के रूप में सामने आते हैं, इसलिए वे 't collide with your template's क्रियाएं कर सकते हैं।",
    },
    {
      lines: "4-6",
      label: "stdio launcher",
      note: "`command` + `args` एक स्थानीय बाइनरी उत्पन्न करता है। Stdio सर्वर **स्थानीय विकास** के लिए हैं - वे एज रनटाइम में नो-ऑप हैं।",
    },
    {
      lines: "6",
      label: "प्रक्रिया पर्यावरण",
...
प्रीसेट आईडी `context7`, `sentry`, `notion`, `semgrep`, `linear`,
`atlassian`, `supabase`, `neon`, `stripe`, `cloudflare`, `gitlab`, `figma`,
`vercel`, `github`, `slack`, `asana`, `hubspot`, `intercom`, `monday`, `webflow`, `paypal`, `box`, `netlify` और `zapier` हैं।

डायरेक्टरी उन एंडपॉइंट्स को अलग दिखाती है जो सामान्य OAuth फ्लो का समर्थन
करते हैं और उन प्रदाताओं को भी जो प्रदाता-अनुमोदित क्लाइंट या पहले से पंजीकृत
OAuth ऐप मांगते हैं। ऐसे प्रदाता खोजने के लिए दिखाई देते हैं और आधिकारिक सेटअप
निर्देशों से जुड़े होते हैं, लेकिन जहां एक-क्लिक कनेक्शन समर्थित नहीं है वहां
भ्रामक बटन नहीं दिखाया जाता। सैंडबॉक्स की गई एक्सटेंशन होस्ट एक्शन के जरिए
`agentNative.mcp.listTools`, `agentNative.mcp.callTool` और
`agentNative.providerApi.catalog(...)`/`agentNative.providerApi.docs(...)` का
उपयोग करके कनेक्टेड डेटा खोज सकती हैं; उन्हें कनेक्शन URL, एक्सेस टोकन या
रीफ्रेश टोकन कभी नहीं मिलते। कच्चा `provider-api-request` जानबूझकर केवल
होस्ट और एजेंट तक सीमित रखा गया है: यदि साझा की गई एक्सटेंशन को मनमाना
मेथड/पाथ अनुरोध करने दिया जाए, तो वह एक्सटेंशन एक ऑथेंटिकेटेड राइट प्रॉक्सी
बन सकती है। इसके बजाय एक्सटेंशन को सीमित-दायरे वाले प्रोवाइडर ऑपरेशन के लिए
`agentNative.mcp.callTool(...)` या किसी खास उद्देश्य के लिए बनाए गए ऐप एक्शन
का उपयोग करना चाहिए।

लोकल-फ़ाइल एक्सटेंशन को `extension.json` में `list-mcp-tools`, `call-mcp-tool`
और उनके द्वारा उपयोग की जाने वाली हर समर्थित प्रोवाइडर कार्रवाई को स्पष्ट रूप
से अनुमति देनी होगी। डिस्कवरी हेल्पर `agentNative.connectors.mcp` और
`agentNative.connectors.providerApi` के तहत भी उपलब्ध हैं।

### Atlassian / Jira

Atlassian प्रीसेट, Atlassian Rovo MCP के OAuth 2.1 एंडपॉइंट
`https://mcp.atlassian.com/v1/mcp/authv2` का उपयोग करता है और Jira तथा Confluence टूल उपलब्ध कराता है। Jira टिकट बनाने के लिए Atlassian की वे अनुमतियां आवश्यक हैं जो Jira में लिखने की अनुमति देती हैं; उपयोगकर्ता को फिर भी केवल वही प्रोजेक्ट और सामग्री दिखाई देती है जिसकी अनुमति उसके Atlassian खाते को है।

Atlassian संगठन के व्यवस्थापक नियंत्रित कर सकते हैं कि किसी OAuth क्लाइंट को कनेक्ट करने की अनुमति है या नहीं। कनेक्ट करने से पहले, किसी व्यवस्थापक को ऐप का वास्तविक HTTPS ओरिजिन **Atlassian Administration → Rovo → Rovo MCP server → Domains** के अंतर्गत विश्वसनीय डोमेन सूची में जोड़ना पड़ सकता है और **Permissions** के अंतर्गत आवश्यक **Read**, **Write** और **Search** अनुमतियां सक्षम करनी पड़ सकती हैं। डोमेन वही होस्ट होना चाहिए जहां agent-native ऐप चलता है; `https://builder.io/**` को किसी अन्य डिप्लॉयमेंट में कॉपी न करें, जब तक कि वह उसका वास्तविक ओरिजिन न हो। Atlassian की [Rovo MCP सेटअप गाइड](https://developer.atlassian.com/cloud/rovo-mcp/guides/getting-started/) और [व्यवस्थापक नियंत्रण](https://support.atlassian.com/security-and-access-policies/docs/control-atlassian-rovo-mcp-server-settings/) देखें।

कैटलॉग एंडपॉइंट और सेटअप निर्देश उपलब्ध कराता है। सामान्य सेटिंग्स फ़ॉर्म एंडपॉइंट और वैकल्पिक एन्क्रिप्टेड हेडर संग्रहीत करता है; यह Atlassian के इंटरैक्टिव OAuth सहमति प्रवाह की जगह नहीं लेता। जब Atlassian को इंटरैक्टिव OAuth की आवश्यकता हो, तो OAuth-सक्षम MCP क्लाइंट या रनटाइम का उपयोग करें। यदि प्रदाता या रनटाइम स्थिर Bearer टोकन का समर्थन करता है, तो उसके अनुमोदित सीक्रेट-मैनेजमेंट तरीके का पालन करें और टोकन को कभी भी prompts या कमिट किए गए कॉन्फ़िगरेशन में पेस्ट न करें।
...
HTTPS URLs हर जगह स्वीकार किए जाते हैं; विकास के दौरान केवल `localhost` के लिए सादे `http://` की अनुमति है। वैकल्पिक प्रमाणीकरण एक बियरर टोकन के रूप में जाता है जो प्रत्येक अनुरोध पर `Authorization: Bearer …` के माध्यम से भेजा जाता है। Atlassian Rovo MCP सहित केवल OAuth का समर्थन करने वाले प्रदाताओं को स्थिर हेडर के बजाय प्रदाता के OAuth प्रवाह की आवश्यकता होती है।
...
<Diagram id="doc-block-cxo2ub" title={"हब मॉडल: एक ऐप ऑर्ग-स्कोप MCP सर्वर पर कार्य करता है"} summary={"Dispatch ऑर्ग-स्कोप MCP सर्वर रखता है; उपभोक्ता ऐप्स उन्हें mcp__hub_<orgId>_<name>__* के रूप में खींचते और मर्ज करते हैं। केवल ऑर्ग-स्कोप पंक्तियाँ साझा की जाती हैं - व्यक्तिगत क्रेडेंशियल यथावत रहते हैं।"}>

```html
<div class="mcp-hub">
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">Dispatch hub</span
    ><small class="diagram-muted">org-scope MCP सर्वर</small
  ...
  <div class="diagram-col consumers">
    ...
    <div class="diagram-box" data-rough>
      Clips<br /><small class="diagram-muted">हर ~60 सेकंड pull + merge</small>
    </div>
  </div>
</div>
...
<Endpoint id="doc-block-1f1u10o" method="GET" path="/_agent-native/mcp/status" summary={"टूलींग और ऑनबोर्डिंग के लिए MCP क्लाइंट स्थिति"} responses={[
  {
    "status": "200",
...
- एक गलत कॉन्फ़िगर किया गया `command` → सर्वर छोड़ दिया गया है, इसकी त्रुटि `errors.<server-id>` के तहत `/_agent-native/mcp/status` में दिखाई देती है, और हर दूसरा सर्वर काम करना जारी रखता है।
