...
> **La mayoría de las personas instalan Plan como una skill, no como una app scaffolded.** Un comando CLI agrega las skills `/visual-plan`, `/visual-recap` y `/visualize-repo`, además del conector Plan alojado, a tu agente de código — consulta [Plan plugin & marketplace](/docs/plan-plugin) para las rutas de plugin y marketplace. Bifurcar la plantilla Plan (cubierto en [For developers](#for-developers)) es la ruta secundaria para autohospedar o construir sobre Plan.
...
<Diagram id="doc-block-17q5b97" title={"Dos comandos, una superficie de revisión"} summary={"Ambos comandos publican a través del conector alojado Plan MCP en la misma superficie de anotaciones y comentarios."}>
...
<div class="diagram-plan">
  <div class="diagram-col">
    <div class="diagram-node">
      ...
      ><small class="diagram-muted"
        >antes del codigo: arquitectura, UI, refactor</small
      >
    </div>
    <div class="diagram-node">
      <span class="diagram-pill">/visual-recap</span
      ><small class="diagram-muted"
        >despues del codigo: PR, commit, rama, diff</small
      >
    </div>
  </div>
  ...
  <div class="diagram-box">
    Superficie de revision<br /><small class="diagram-muted"
      >diagramas · wireframes · codigo anotado · comentarios</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&harr;</div>
  <div class="diagram-node">
    Agente de codigo<br /><small class="diagram-muted">feedback devuelto</small>
  </div>
</div>
```
...
<WireframeBlock id="doc-block-vi5rkz">
  <Screen
    surface="desktop"
    html={
...
El comando instala tres comandos: `/visual-plan`, `/visual-recap` y `/visualize-repo`.
...
`https://plan.agent-native.com/mcp`: consulte [MCP Clients](/docs/mcp-clients) para conocer la configuración específica del cliente.
...
## Costo y elección de modelo

Generar un plan o un resumen ejecuta una sesión completa de agente frente al LLM
de su agente de codificación, no un simple relleno de plantilla ligero, por lo que
el costo escala con el tamaño del diff o del plan y con el modelo que elija.

- El modelo de chat predeterminado de la aplicación es GPT-5.6 Luna. Las
  conexiones directas con Anthropic mantienen Claude Sonnet 5 como modelo
  predeterminado. Cámbielo en cualquier momento en **Configuración**; Haiku es
  la opción más económica de Claude.
- Los resúmenes de PR automatizados de la GitHub Action se ejecutan por separado
  de la aplicación interactiva y tienen sus propios controles de modelo y costo;
  consulte [PR Visual Recap](/docs/pr-visual-recap) para `VISUAL_RECAP_MODEL`,
  `VISUAL_RECAP_AGENT` y `VISUAL_RECAP_REASONING`.
...
El conector de planes alojado se encuentra en `https://plan.agent-native.com/mcp`.
...
## Documentación visual del repositorio {#visualize-repo}

Usa `/visualize-repo` cuando quieras una capa duradera de documentación visual para una base de código en lugar de un plan o recapitulación puntual. Crea o abre un workspace local de Plan MDX, actualiza `agent-native.json` con una sección `apps.visualize-repo` en modo local-files y sirve los documentos a través del mismo puente localhost:

```bash
npx @agent-native/core@latest visualize-repo --open
```

Si no existe `agent-native.json`, el comando lo inicializa y recomienda un pequeño conjunto inicial a partir de la estructura del repositorio, como `actions/`, `app/components/`, `app/pages/`, `server/db/schema.ts`, `src/`, `packages/`, `templates/`, `docs/` o `content/`. Puedes apuntar a anclas de código específicas:
...
La carpeta inicial vive de forma predeterminada en `.agent-native/visual-docs/repo-overview`. Los comentarios permanecen locales en `comments.json`, para que los revisores puedan marcar secciones para un agente de código y el agente pueda actualizar código y MDX en el mismo cambio. Ejecuta `visualize-repo check` para lint local y `visualize-repo verify` cuando quieras que el esquema del renderizador de Plan valide el MDX.
...
- "Ejecuta `/visualize-repo` y empieza con nuestras acciones API públicas y el esquema."
...
<DataModel
  id="doc-block-1mj35pp"
  title="Modelo de datos Plan"
  summary={
    "Una fila del plan posee secciones ordenadas además de comentarios, eventos, versiones, recursos compartidos y activos en línea."
  }
  entities={[
    {
      id: "plans",
