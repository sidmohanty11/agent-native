...
<Endpoint id="doc-block-1wnwsyu" title="Lista de notificaciones" summary={"The route behind listNotifications() — scoped to the authenticated session's email."} method="GET" path={"/_agent-native/notifications?unread=true&limit=50"} summary="Listar notificaciones recientes para el usuario actual" auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
