...
description: "Instala las skills de Agent-Native Plan (/visual-plan, /visual-recap, /visualize-repo) más el conector alojado de Plan MCP como complemento de Claude Code o Codex, o con el CLI universal. Cómo funcionan las actualizaciones y si necesitas enviar algo."
---

# Complemento y mercado del plan

La aplicación **Plan** de Agent-Native se envía como un paquete instalable. Una sola instalación agrega las skills de comandos slash de Plan **y** conecta el conector alojado de Plan MCP, de modo que el agente pueda generar planes y las skills puedan publicarlos directamente en la aplicación Plan.
...
- **El conector del plan MCP**: registrado en la aplicación alojada en `https://plan.agent-native.com` (extremo MCP `https://plan.agent-native.com/mcp`, nombre de servidor `plan`).

<Diagram id="doc-block-1i4grvb" title="Tres rutas, un paquete" summary={"El CLI universal, el complemento de Claude Code y el complemento de Codex instalan las mismas skills de Plan más el conector de Plan alojado."}>
...
De forma predeterminada, `/visual-plan` y `/visual-recap` publican en la aplicación Plan alojada: crean un plan a través de
...
<Diagram id="doc-block-1glivuy" title="Modo alojado frente a archivos locales" summary={"De forma predeterminada, las habilidades se publican a través del conector; El modo de archivos locales escribe MDX en el disco y, en su lugar, realiza una vista previa a través de un puente de host local."}>
...
<div class="diagram-modes">
  ...
  <div class="diagram-card">
    ...
    ><small class="diagram-muted"
      ...
      hosted Plan UI reads local source. No Escritura en BDs until
...
Funciona con cualquier host: Claude Code, Codex, Cursor, Cline, Goose, aplicaciones MCP personalizadas de ChatGPT, Claude Cowork y cualquier otra cosa compatible con MCP. El CLI de Agent-Native instala las skills de Plan, registra el conector alojado de Plan MCP **y ejecuta la autenticación para los clientes locales seleccionados en el mismo paso**, para que tu primera llamada a la herramienta no llegue a un muro de OAuth:

```bash
npx @agent-native/core@latest skills add visual-plan
```

Esto instala `visual-plan` más las skills complementarias `visual-recap` y `visualize-repo`, luego registra el conector `plan` y ejecuta la autenticación (solicitud de OAuth para uso compartido alojado/respaldado por cuenta). Flags útiles:
...
`/plugin install` agrega las skills de Plan y una configuración MCP **solo de URL** (sin secretos en el paquete); `/mcp` → **Authenticate** completa el handshake de OAuth. Usa la ruta del CLI universal cuando quieras archivos locales o modo autohospedado.
