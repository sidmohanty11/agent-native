  ...
  <div class="diagram-col sources">
    <div class="diagram-box" data-rough>
      Workspace <code>mcp.config.json</code><br /><small class="diagram-muted"
        >compartido entre apps</small
      >
    </div>
    <div class="diagram-box" data-rough>
      Raíz de la app <code>mcp.config.json</code><br /><small
        class="diagram-muted"
        >sobrescritura por app</small
      >
    </div>
    <div class="diagram-box" data-rough>
    ...
    <div class="diagram-box" data-rough>
      Remoto desde la UI de ajustes<br /><small class="diagram-muted"
...
<AnnotatedCode
  ...
  annotations={[
    {
      lines: "3",
      label: "identificación del servidor",
      note: "La clave se convierte en el prefijo de la herramienta: las herramientas de este servidor aparecen como `mcp__claude-in-chrome__*` en el registro del agente, por lo que no pueden colisionar con las acciones de su plantilla.",
    },
    {
      lines: "4-6",
      label: "stdio launcher",
      note: "`command` + `args` generan un binario local. Los servidores Stdio están destinados al **desarrollo local**; no son operativos en los tiempos de ejecución perimetrales.",
    },
    {
      lines: "6",
      label: "entorno de proceso",
...
Los identificadores de los ajustes preestablecidos son `context7`, `sentry`, `notion`, `semgrep`, `linear`,
`atlassian`, `supabase`, `neon`, `stripe`, `cloudflare`, `gitlab`, `figma`,
`vercel`, `github`, `slack`, `asana`, `hubspot`, `intercom`, `monday`, `webflow`, `paypal`, `box`, `netlify` y `zapier`.

El directorio distingue los puntos de conexión que admiten el flujo OAuth
genérico de los proveedores que requieren un cliente aprobado por el proveedor
o una aplicación OAuth registrada previamente. Estos últimos siguen visibles
para su descubrimiento y enlazan a las instrucciones oficiales, pero no muestran
un botón de conexión de un clic que pueda resultar engañoso. Las extensiones
aisladas usan `agentNative.mcp.listTools`, `agentNative.mcp.callTool` y
`agentNative.providerApi.catalog(...)`/`agentNative.providerApi.docs(...)` a
través de las acciones del host para descubrir datos conectados; nunca reciben
URL de conexión, tokens de acceso ni tokens de actualización. La llamada
directa `provider-api-request` se reserva intencionadamente al host y al
agente: exponer solicitudes arbitrarias de método/ruta a extensiones
compartidas convertiría a una extensión en un proxy de escritura autenticado.
En su lugar, las extensiones deben usar `agentNative.mcp.callTool(...)` o una
acción de la aplicación creada para ese fin, limitada a operaciones concretas
del proveedor.

Las extensiones locales deben permitir explícitamente `list-mcp-tools`,
`call-mcp-tool` y cualquier acción de proveedor compatible que utilicen en
`extension.json`. Los helpers de descubrimiento también están disponibles en
`agentNative.connectors.mcp` y `agentNative.connectors.providerApi`.

### Atlassian / Jira

El ajuste preestablecido de Atlassian usa el punto de conexión OAuth 2.1 de Atlassian Rovo MCP,
`https://mcp.atlassian.com/v1/mcp/authv2`, y proporciona herramientas de Jira y Confluence. Para crear incidencias de Jira se necesitan los permisos de Atlassian que permiten escribir en Jira; el usuario solo puede ver los proyectos y el contenido a los que su cuenta de Atlassian tiene acceso.

Los administradores de una organización de Atlassian pueden controlar si un cliente OAuth tiene permiso para conectarse. Antes de establecer la conexión, es posible que un administrador deba añadir el origen HTTPS real de la aplicación a la lista de dominios de confianza en **Administración de Atlassian → Rovo → Servidor Rovo MCP → Dominios** y habilitar los permisos necesarios de **Lectura**, **Escritura** y **Búsqueda** en **Permisos**. El dominio debe ser el host en el que se ejecuta la aplicación agent-native; no copie `https://builder.io/**` en otro despliegue salvo que ese sea su origen real. Consulte la [guía de configuración de Rovo MCP](https://developer.atlassian.com/cloud/rovo-mcp/guides/getting-started/) y los [controles de administración](https://support.atlassian.com/security-and-access-policies/docs/control-atlassian-rovo-mcp-server-settings/) de Atlassian.

El catálogo proporciona el punto de conexión y las instrucciones de configuración. El formulario de configuración genérico guarda el punto de conexión y los encabezados cifrados opcionales, pero no sustituye el flujo de consentimiento OAuth interactivo de Atlassian. Use un cliente o un entorno de ejecución MCP compatible con OAuth cuando Atlassian requiera OAuth interactivo. Si el proveedor o el entorno de ejecución admite un token Bearer estático, siga el método aprobado para administrar secretos y nunca pegue tokens en prompts ni en archivos de configuración enviados al repositorio.
...
HTTPS URL se aceptan en todas partes; `http://` simple solo se permite para `localhost` durante el desarrollo. La autenticación opcional se ingresa como un token de portador que se envía a través de `Authorization: Bearer …` en cada solicitud. Los proveedores que solo admiten OAuth, incluido Atlassian Rovo MCP, requieren el flujo OAuth del proveedor en lugar de un encabezado estático.
...
<div class="mcp-hub">
  <div class="diagram-panel center" data-rough>
    ...
    ><small class="diagram-muted"
      >servidores MCP con alcance de organización</small
    ><small class="diagram-muted"
  ...
  <div class="diagram-col consumers">
    ...
    <div class="diagram-box" data-rough>
      Clips<br /><small class="diagram-muted">pull + merge cada ~60 s</small>
    </div>
  </div>
</div>
...
<Endpoint id="doc-block-1t9ze0r" method="GET" path="/_agent-native/mcp/status" summary={"Estado del cliente MCP para herramientas e incorporación"} responses={[
  {
    "status": "200",
...
- Un `command` mal configurado → se omite el servidor, su error aparece en `/_agent-native/mcp/status` debajo de `errors.<server-id>` y todos los demás servidores continúan funcionando.
