...
<Diagram id="doc-block-1rwsxoo" title={"El padre guarda los secretos; el adaptador solo ejecuta código"} summary={"run-code construye el módulo y ejecuta el puente loopback; el adaptador recibe un módulo no secreto + límites y devuelve stdout/stderr/exit."}>

```html
<div class="diagram-sandbox">
  <div class="diagram-box" data-rough>
    <strong>Proceso padre</strong
    ><small class="diagram-muted"
      >compila módulo · puente loopback · limpieza de entorno · formato de
      salida</small
    >
  </div>
  ...
  <div class="diagram-panel center" data-rough>
    ...
    ><small class="diagram-muted"
      >proceso hijo local · Docker · remoto · duradero</small
    >
  </div>
</div>
```
...
<FileTree
  id="doc-block-e16klt"
  title={"El punto de unión del sandbox en core"}
  title="packages/core/src/coding-tools/sandbox/"
  entries={[
    {
      path: "adapter.ts",
      note: "El contrato SandboxAdapter (SandboxRunRequest / SandboxRunResult)",
    },
    {
      path: "index.ts",
    ...
    {
      path: "../run-code.ts",
      note: "Conecta ese punto; no cambia al sustituir backends",
    },
  ]}
/>
