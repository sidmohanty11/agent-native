...
<Endpoint id="doc-block-12cjrag" method="POST" path="/_agent-native/mcp" summary="Punto final de puerta de enlace MCP unificada" auth="Standard remote MCP OAuth, handled by the framework. The granted-app set scopes which workspace apps the connector can reach." responses={[
  {
    "status": "200",
