...
> **ほとんどの人は、scaffolded app ではなく skill として Plan をインストールします。** 1 つの CLI コマンドで、`/visual-plan`、`/visual-recap`、`/visualize-repo` skills とホスト型 Plan コネクタが coding agent に追加されます。plugin と marketplace のルートは [Plan plugin & marketplace](/docs/plan-plugin) を参照してください。Plan template の fork（[For developers](#for-developers) で説明）は、self-hosting または Plan 自体の上に構築するための secondary path です。
...
<Diagram id="doc-block-138ilrn" title={"2 つのコマンド、1 つのレビュー サーフェス"} summary={"どちらのコマンドも、ホストされた Plan MCP コネクタを介して、同じ注釈とコメントのサーフェスにパブリッシュされます。"}>
...
<div class="diagram-plan">
  ...
  <div class="diagram-panel center">
    Plan MCP コネクター<br /><small class="diagram-muted"
...
<WireframeBlock id="doc-block-go0l7e">
  <Screen
    surface="desktop"
    html={
...
このコマンドは、`/visual-plan`、`/visual-recap`、`/visualize-repo` の 3 つのコマンドをインストールします。

MCP コネクタ URL を直接受け入れるチャットベースのホストを使用している場合
(CLI で構成されたクライアントではなく)、ホストされたプラン コネクタに
`https://plan.agent-native.com/mcp` — クライアント固有のセットアップについては、[MCP Clients](/docs/mcp-clients) を参照してください。
...
プランや要約の生成は、軽量なテンプレート埋め込みではなく、コーディング エージェントの LLM に対するフルエージェント型セッションを実行するため、コストは差分やプランのサイズと選択したモデルに応じて変わります。

- アプリのデフォルト チャット モデルは GPT-5.6 Luna です。Anthropic に直接接続する場合は、引き続き Claude Sonnet 5 がデフォルトです。モデルはいつでも **[設定]** で変更でき、Haiku はより安価な Claude の選択肢です。
- GitHub Action による自動 PR 要約は、インタラクティブなアプリとは別に実行され、独自のモデルとコストの調整項目を持ちます。`VISUAL_RECAP_MODEL`、`VISUAL_RECAP_AGENT`、`VISUAL_RECAP_REASONING` については [PR Visual Recap](/docs/pr-visual-recap) を参照してください。
...
ホストされたプラン コネクタは `https://plan.agent-native.com/mcp` にあります。
...
## リポジトリのビジュアル ドキュメント {#visualize-repo}

一度きりのプランや recap ではなく、コードベースに永続的なビジュアル ドキュメント層が必要なときは `/visualize-repo` を使います。ローカルの Plan MDX workspace を作成または開き、`agent-native.json` に `apps.visualize-repo` の local-files セクションを追加し、同じ localhost ブリッジ経由で docs を表示します。

```bash
npx @agent-native/core@latest visualize-repo --open
```

`agent-native.json` がない場合、このコマンドはそれを初期化し、repo 構造から `actions/`、`app/components/`、`app/pages/`、`server/db/schema.ts`、`src/`、`packages/`、`templates/`、`docs/`、`content/` などの小さな開始セットを推奨します。特定の source anchor を指定することもできます。
...
スターター フォルダーはデフォルトで `.agent-native/visual-docs/repo-overview` に置かれます。コメントは `comments.json` にローカル保存されるため、レビュアーは coding agent 向けにセクションをマークでき、agent は同じ変更で code と MDX の両方を更新できます。ローカル lint には `visualize-repo check` を実行し、Plan renderer schema で MDX を検証したいときは `visualize-repo verify` を実行します。
...
- 「`/visualize-repo` を実行し、公開 API アクションとスキーマから始めてください。」
...
<DataModel
  ...
  title={"計画データモデル"}
  summary={
    "1 つのプラン行には、順序付けされたセクションに加えて、コメント、イベント、バージョン、共有、およびインライン アセットが含まれます。"
  }
  entities={[
    {
      id: "plans",
