...
description: "Agent-Native Plan skills（/visual-plan、/visual-recap、/visualize-repo）とホスト型 Plan MCP コネクタを、Claude Code または Codex プラグインとして、またはユニバーサル CLI でインストールします。更新の仕組みと、何かを送信する必要があるかどうか。"
---

# プラグインとマーケットプレイスの計画

Agent-Native **Plan** アプリは、1 つのインストール可能なバンドルとして提供されます。1 回のインストールで Plan の slash-command skills が追加され、ホスト型 Plan MCP コネクタも接続されるため、agent は plans を生成でき、skills はそれらを Plan アプリに直接公開できます。

## 得られるもの {#what-you-get}

1 回のインストールで次のことが可能になります:

- **3 つの skills** — `/visual-plan`（正規のエントリ ポイント）、`/visual-recap`、`/visualize-repo`。
- **プラン MCP コネクタ** — `https://plan.agent-native.com` (MCP エンドポイント `https://plan.agent-native.com/mcp`、サーバー名 `plan`) でホストされているアプリに対して登録されています。

<Diagram id="doc-block-istfxt" title={"3つのルート、1つのバンドル"} summary={"ユニバーサル CLI、Claude Code プラグイン、Codex プラグインはいずれも、同じ Plan skills とホスト型 Plan コネクタをインストールします。"}>
...
<div class="diagram-routes">
  <div class="diagram-col">
    ...
    <div class="diagram-node">
      Claude Codeプラグイン<br /><small class="diagram-muted"
        >/plugin install</small
      >
    </div>
    <div class="diagram-node">
      Codexプラグイン<br /><small class="diagram-muted">codex plugin add</small>
    </div>
  </div>
...
デフォルトでは、`/visual-plan` と `/visual-recap` はホスト型 Plan アプリに公開します — MCP コネクタ経由で plan を作成し、
...
<Diagram id="doc-block-1isj86" title={"ホスト ファイル モードとローカル ファイル モード"} summary={"デフォルトでは、スキルはコネクタを通じて公開されます。ローカル ファイル モードは、MDX をディスクに書き込み、代わりにローカルホスト ブリッジ経由でプレビューします。"}>

```html
<div class="diagram-modes">
  <div class="diagram-card">
    <span class="diagram-pill accent">デフォルト · ホスト済み</span
    ><strong>Planアプリに公開</strong
  ...
  <div class="diagram-card">
    <span class="diagram-pill warn">ローカルファイルのプライバシー</span
    ><strong>MDX をディスクに書き込む</strong
    ><small class="diagram-muted"
      >plan.mdx + canvas.mdx + prototype.mdx &rarr; localhost bridge &rarr;
      hosted Plan UI reads local source. No DB 書き込みs until
...
Claude Code、Codex、Cursor、Cline、Goose、ChatGPT custom MCP apps、Claude Cowork、その他 MCP-compatible な任意の host で動作します。Agent-Native CLI は Plan skills をインストールし、ホスト型 Plan MCP コネクタを登録し、**選択したローカル client の認証を同じ手順で実行する**ため、最初の tool call が OAuth の壁に当たりません。

```bash
npx @agent-native/core@latest skills add visual-plan
```

これにより、`visual-plan` と companion skills の `visual-recap`、`visualize-repo` がインストールされ、`plan` コネクタが登録され、認証が実行されます（hosted/account-backed sharing 用の OAuth prompt）。便利な flags:
...
`/plugin install` は Plan skills と **URL-only** MCP config（package 内に secrets はありません）を追加します。`/mcp` → **Authenticate** で OAuth handshake が完了します。local-files または self-hosted mode が必要な場合は、代わりに universal CLI route を使ってください。
