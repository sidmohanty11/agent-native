...
<Diagram id="doc-block-xco4gl" title={"Claude-Code ワークスペースですが、SQL に保存されます"} summary={"同じカスタマイズ層 (命令、スキル、メモリ、エージェント、ジョブ、MCP) ですが、すべてのファイルが共有マルチテナント データベース内の行である点が異なります。"}>
...
<div class="ws-map">
  ...
  <div class="diagram-card an">
    <span class="diagram-pill accent">Agent-nativeワークスペース</span
...
<DataModel
  id="doc-block-vudfrb"
  title={"ワークスペースのリソースモデル"}
  summary={
    "1 つのリソース シェイプがすべてのワークスペース ファイルをサポートします。ランタイムはパスとスコープによってキーを設定し、読み取り時に有効な値を解決します。"
  }
  entities={[
    {
      id: "resource",
