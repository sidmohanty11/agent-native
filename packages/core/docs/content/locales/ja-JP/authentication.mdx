...
<Diagram id="doc-block-76smlp" title={"1 回のセッションで 3 つの方法で"} summary={"ブラウザーの訪問者、プログラムによる MCP クライアント、およびカスタム プロバイダーはすべて、ダウンストリーム スコーピングが読み取る同じ AuthSession に解決されます。"}>
