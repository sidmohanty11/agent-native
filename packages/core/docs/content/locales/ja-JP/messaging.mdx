...
- 同じスレッドでの通常の追加返信がエージェントに届くのは、**そのスレッドで実行中の作業が残っている間だけ**です。実行が終わっていれば追加の返信は無視されるため、まだ聞いていると仮定せずに改めて @mention してください。
...
| 一度は成功したが、その後のスレッド返信が無視された        | 想定どおりの動作です。メンションなしのスレッド返信は、そのスレッドに実行中の作業がある間しかエージェントに届きません。改めて @mention してください。    |
