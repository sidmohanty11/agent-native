...
<AnnotatedCode
  id="doc-block-1wo6u59"
  title={"1 つの結果的なアクションをゲートする"}
  ...
  annotations={[
    {
      lines: "10",
      label: "門全体",
      note: "フラグは 1 つです。それが真実であり、呼び出しが承認されていない場合、ループは `run` の前で停止します。つまり、モデル自体が副作用に到達することはありません。",
    },
    {
      lines: "11-13",
      label: "run() is untouched",
      note: "ハンドラーはそのままです。承認は、`run` 内の何かによってではなく、周囲のループによって強制されます。",
    },
  ]}
/>
...
<Diagram id="doc-block-184e1ec" title={"承認の中断"} summary={"ゲート呼び出しは、run() が起動される前にターンを一時停止します。承認により、コールのキーを保持するターンが再発行されます。そうして初めて副作用が起こります。"}>

```html
<div class="diagram-approve">
  <div class="diagram-box" data-rough>エージェントがsend-emailを呼び出す</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-panel warn" data-rough>
    <strong>Gateはtrue、呼び出しは未承認</strong
  ...
  <div class="diagram-box" data-rough>
    人がチャットで承認をクリック<br /><small class="diagram-muted"
      >クライアントがapprovedToolCalls: [approvalKey]でターンを再発行</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
