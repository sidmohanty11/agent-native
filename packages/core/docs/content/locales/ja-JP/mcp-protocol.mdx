...
- **自動マウント** — すべてのアプリは無料で `/mcp` を入手でき、セットアップは必要ありません
...
<Endpoint id="doc-block-14spw6g" title={"MCP エンドポイント"} summary={"すべてのエージェント ネイティブ アプリが公開する、自動マウントされる Streamable HTTP エンドポイント。"} method="POST" path="/mcp" summary={"MCP ストリーミング可能な HTTP エンドポイント"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
ホストされている MCP リクエストは境界のあるインライン待機を使用するため、長時間のエージェント ループが 1 つの `tools/call` をクライアントまたはゲートウェイのタイムアウトまで開いたままにすることはありません。オプションの `async` 引数を指定すると、永続的なタスクが開始されすぐに応答が返されます。指定しない場合、サーバーは短時間だけ高速な完了を待ってから、処理が残っている場合はタスク ハンドルを返します。ホストされている待機は 25 秒に制限されています。
...
エージェントは対話型チャットと同じループを実行します。エージェントは複数のツールを呼び出し、コンテキストを推論し、思慮深い応答を生成できます。完了していないタスクは、`taskId`、`status`、`pollAfterMs` を含む JSON を返します。
...
| `/mcp/oauth/register`                     | 動的パブリッククライアント登録          |
| `/mcp/oauth/authorize`                    | ブラウザの承認 + 同意                   |
| `/mcp/oauth/token`                        | 認可コードとリフレッシュトークンの付与  |
