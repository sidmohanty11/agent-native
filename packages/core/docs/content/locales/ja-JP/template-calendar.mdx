...
<Diagram id="doc-block-12iygmb" title={"スケジュールリクエストの流れ"} summary={"カレンダー内をクリックするか、エージェントに問い合わせるかにかかわらず、同じアクションが Google Calendar からライブで読み込まれ、同じビューに書き戻されます。"}>
...
<div class="diagram-flow">
  ...
  <div class="diagram-box">カレンダー表示がライブ更新されます</div>
</div>
```
...
<DataModel
  ...
  title={"カレンダーデータモデル"}
  summary={
    "イベント以外のデータのみがローカルに保存されます。イベントは Google カレンダーに保存されます。予約リンクは ownableColumns を使用するため、共有システムが適用されます。"
  }
  entities={[
    {
      id: "booking_links",
