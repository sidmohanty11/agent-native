...
<Diagram id="doc-block-1i1rytr" title={"/merge はコード セッションであり、別個のアプリではありません"} summary={"パス、URL、または説明が入ります。実行では、他のすべてのコード セッションと同じストア、トランスクリプト、およびコントロールを共有し、ポータブルなドシエを出力できます。"}>
...
<div class="diagram-migrate">
  ...
  <div class="diagram-panel center" data-rough>
    ...
    ><small class="diagram-muted"
      >同じストア · トランスクリプト · 実行制御</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-col">
    <div class="diagram-box" data-rough>移行済みアプリ</div>
