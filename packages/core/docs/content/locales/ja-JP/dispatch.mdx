...
<Endpoint id="doc-block-1hsfyib" method="POST" path="/_agent-native/mcp" summary={"統合 MCP ゲートウェイ エンドポイント"} auth="Standard remote MCP OAuth, handled by the framework. The granted-app set scopes which workspace apps the connector can reach." responses={[
  {
    "status": "200",
