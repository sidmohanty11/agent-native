...
> **Die meisten Leute installieren Plan als Skill und nicht als Gerüst-App.** Ein CLI-Befehl fügt die Skills `/visual-plan`, `/visual-recap` und `/visualize-repo` sowie den gehosteten Plan-Connector zu Ihrem Codierungsagenten hinzu — siehe [Plan plugin & marketplace](/docs/plan-plugin) für die Plugin- und Marktplatzrouten. Das Forken der Planvorlage (unter [For developers](#for-developers)) ist der sekundäre Pfad für Selbsthosting oder das Bauen auf Plan selbst.
...
<Diagram id="doc-block-176dlsp" title={"Zwei Befehle, eine Überprüfungsoberfläche"} summary={"Beide Befehle veröffentlichen über den gehosteten Plan MCP-Connector in derselben Anmerkungs- und Kommentaroberfläche."}>
...
<div class="diagram-plan">
  <div class="diagram-col">
    <div class="diagram-node">
      ...
      ><small class="diagram-muted"
        >vor dem Code: Architektur, UI, Refactor</small
      >
    </div>
    <div class="diagram-node">
  ...
  <div class="diagram-box">
    Review-Oberflaeche<br /><small class="diagram-muted"
      >Diagramme · Wireframes · annotierter Code · Kommentare</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&harr;</div>
  <div class="diagram-node">
    Coding Agent<br /><small class="diagram-muted"
      >Feedback zurueckgegeben</small
    >
  </div>
</div>
```
...
<WireframeBlock id="doc-block-13e4f0r">
  <Screen
    surface="desktop"
    html={
...
Der Befehl installiert drei Befehle: `/visual-plan`, `/visual-recap` und `/visualize-repo`.
...
`https://plan.agent-native.com/mcp` – siehe [MCP Clients](/docs/mcp-clients) für kundenspezifische Einrichtung.
...
- Das Standard-Chat-Modell der App ist GPT-5.6 Luna. Direkte Anthropic-Verbindungen verwenden weiterhin Claude Sonnet 5 als Standard. Sie können das Modell jederzeit in den **Einstellungen** ändern; Haiku ist die günstigere Claude-Option.
...
Der gehostete Plan-Connector befindet sich unter `https://plan.agent-native.com/mcp`.
...
## Visuelle Repo-Dokumentation {#visualize-repo}

Verwenden Sie `/visualize-repo`, wenn Sie eine dauerhafte visuelle Dokumentationsschicht für eine Codebase statt eines einmaligen Plans oder Recaps wollen. Der Befehl erstellt oder öffnet einen lokalen Plan-MDX-Arbeitsbereich, aktualisiert `agent-native.json` mit einem `apps.visualize-repo`-Abschnitt im local-files-Modus und stellt die Dokumentation über dieselbe localhost-Bridge bereit:

```bash
npx @agent-native/core@latest visualize-repo --open
```

Wenn kein `agent-native.json` existiert, legt der Befehl eines an und empfiehlt aus der Repo-Struktur einen kleinen Startsatz, zum Beispiel `actions/`, `app/components/`, `app/pages/`, `server/db/schema.ts`, `src/`, `packages/`, `templates/`, `docs/` oder `content/`. Sie können bestimmte Source-Anker angeben:
...
Der Starter-Ordner liegt standardmäßig unter `.agent-native/visual-docs/repo-overview`. Kommentare bleiben lokal in `comments.json`, sodass Reviewer Abschnitte für einen Coding-Agent markieren können und der Agent Code und MDX in derselben Änderung aktualisieren kann. Führen Sie `visualize-repo check` für lokales Linting aus und `visualize-repo verify`, wenn das Plan-Renderer-Schema das MDX validieren soll.
...
- „Führen Sie `/visualize-repo` aus und beginnen Sie mit unseren öffentlichen API-Actions und dem Schema.“
...
<DataModel
  id="doc-block-1etdmmd"
  title="Plan Datenmodell"
  summary={
    "Eine Planzeile enthält geordnete Abschnitte sowie Kommentare, Ereignisse, Versionen, Freigaben und Inline-Assets."
  }
  entities={[
    {
      id: "plans",
