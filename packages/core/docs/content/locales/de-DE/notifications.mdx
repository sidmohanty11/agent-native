...
<Endpoint id="doc-block-fro2p7" title="Benachrichtigungen auflisten" summary={"The route behind listNotifications() — scoped to the authenticated session's email."} method="GET" path={"/_agent-native/notifications?unread=true&limit=50"} summary={"Letzte Benachrichtigungen für den aktuellen Benutzer auflisten"} auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
