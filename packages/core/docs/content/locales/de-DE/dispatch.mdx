...
<Endpoint id="doc-block-1gi4v43" method="POST" path="/_agent-native/mcp" summary="Einheitlicher MCP-Gateway-Endpunkt" auth="Standard remote MCP OAuth, handled by the framework. The granted-app set scopes which workspace apps the connector can reach." responses={[
  {
    "status": "200",
