...
<Diagram id="doc-block-1y7a55" title="Gabeln und anpassen" summary={"Wählen Sie ein fertiges Produkt aus, versehen Sie es mit einem Branding, entwickeln Sie es in einfachem Englisch weiter und versenden Sie es an Ihre eigene Domain."}>
...
<Diagram id="doc-block-tslhp4" title="Warum Forks pro Benutzer skalieren" summary={"Zwei Ideen sorgen dafür, dass das Fork-and-Customize-Modell praktisch bleibt: Der Agent übernimmt die Wartung und die benutzerspezifische Anpassung erfolgt in SQL – nicht im benutzerspezifischen Code."}>
