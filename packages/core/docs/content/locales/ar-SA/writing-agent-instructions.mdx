...
{
  "html": "<div class=\"diagram-surfaces\"><div class=\"diagram-card always\" data-rough><span class=\"diagram-pill accent\">كل دور</span><strong>AGENTS.md</strong><small class=\"diagram-muted\">الخريطة: الغرض، القواعد الأساسية، مفاتيح الحالة، فهرس الإجراءات + المهارات</small></div><div class=\"diagram-card always\" data-rough><span class=\"diagram-pill accent\">كل دور</span><strong>أوصاف الأدوات</strong><small class=\"diagram-muted\">توجه اختيار الأدوات — جملة دقيقة لكل أداة</small></div><div class=\"diagram-card ondemand\" data-rough><span class=\"diagram-pill\">عند الطلب</span><strong>المهارات</strong><small class=\"diagram-muted\">إرشادات معمقة، تحمل عندما ينطبق الوصف</small></div><div class=\"diagram-card runtime\" data-rough><span class=\"diagram-pill ok\">Live</span><strong>application_state</strong><small class=\"diagram-muted\">تكتبه واجهتك: التنقل، التحديد، التركيز</small></div></div>",
...
- **فهرس المهارات** — قائمة بـ skills الموجودة ومتى يجب قراءة كل منها.
...
- Data lives in SQL عبر Drizzle. Use actions for all writes.
...
├── SKILL.md            # القاعدة + المسار السعيد + do/don't
└── references/
    └── csv-format.md   # مواصفات العمود الكامل، والترميزات، وحالات الحافة
```

يؤدي ذلك إلى إبقاء السطح الذي يتم تحميله دائمًا صغيرًا ويسمح بقياس العمق دون انتفاخ السياق. راجع [المهارات Guide](/docs/skills-guide) للحصول على تنسيق المهارات الكاملة.
...
## المهارات ضد actions {#skills-vs-actions}

المهارات وactions متكاملان. المهارة هي التوجيه الذي يقرأه الوكيل؛
...
| المهارات (`SKILL.md`)    | أنت (المطور)                     | عند الطلب عندما يقرر الوكيل أن المهارة ذات صلة | كيفية تنفيذ نمط معين خطوة بخطوة، وقوائم المهام المحظورة           |
...
- **المهارات** — طريقة عمل قابلة لإعادة الاستخدام لنمط معين، ويتم تحميلها عند الطلب. ينطبق على جميع العاملين في التطبيق.
- **الذاكرة (`memory/MEMORY.md`)** — التفضيلات والتصحيحات لكل مستخدم، وليس توجيهات المؤلف.

## ما هي الخطوة التالية {#whats-next}

- [المهارات Guide](/docs/skills-guide) — تنسيق ملف المهارات، وإطار العمل skills، وskills المدعوم بالتطبيق.
