...
- **يتم التثبيت تلقائيًا** — يحصل كل تطبيق على `/mcp` مجانًا، دون الحاجة إلى أي إعداد
...
| **نقطة النهاية**  | `/mcp`                                                                  | `/_agent-native/a2a`                           |
...
<Endpoint id="doc-block-17mmuo8" title={"MCP نقطة النهاية"} summary={"نقطة نهاية Streamable HTTP المثبتة تلقائيًا والتي يكشفها كل تطبيق أصلي للوكيل."} method="POST" path="/mcp" summary={"MCP نقطة النهاية HTTP القابلة للبث"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
تستخدم طلبات MCP المستضافة انتظارًا مضمّنًا محدودًا حتى لا تُبقي حلقة وكيل طويلة استدعاء `tools/call` واحدًا مفتوحًا حتى انتهاء مهلة العميل أو البوابة. تبدأ الوسيطة الاختيارية `async` المهمة الدائمة وتُرجع فورًا؛ وبدونها، ينتظر الخادم لفترة قصيرة إتمامًا سريعًا ثم يُرجع معرّف مهمة عند تبقّي المزيد من العمل. يبلغ الحد الأقصى للانتظار المستضاف 25 ثانية.
...
يُدير الوكيل نفس الحلقة مثل الدردشة التفاعلية — يمكنه استدعاء أدوات متعددة، والتفكير في السياق، وتقديم استجابة مدروسة. تُرجع المهمة غير المكتملة JSON يحتوي على `taskId` و`status` و`pollAfterMs`، بالإضافة إلى:
...
| `/mcp/oauth/register`                     | التسجيل الديناميكي للعميل العام             |
| `/mcp/oauth/authorize`                    | تخويل المتصفح + الموافقة                    |
| `/mcp/oauth/token`                        | منح رمز التفويض ورمز التحديث                |
