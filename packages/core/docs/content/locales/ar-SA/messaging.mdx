...
- **إشارات القناة** — استخدم @mention لبدء العمل. تُقبل الردود بلا إشارة فقط ما دام العمل نشطًا في سلسلة الرسائل نفسها.
...
OAuth المُدار هو الإعداد الموصى به. من **الإعدادات → المراسلة**، اختر
...
و`message.channels` و`message.groups` و`message.im` و`message.mpim`.
بعد إعداد `SLACK_CLIENT_ID` و`SLACK_CLIENT_SECRET`
و`SLACK_SIGNING_SECRET`، افتح **الإعدادات → المراسلة** وانقر **Add to Slack**.
...
أثناء تشغيل مهمة في سلسلة رسائل، تواصل الردود غير المشار إليها المهمة نفسها؛
ولا تبدأ محادثات القناة العامة مهمة. يحمّل الوكيل سياقًا محدودًا مع التفاعلات
...
- الرد العادي اللاحق في السلسلة نفسها يصل إلى الوكيل **فقط ما دام لتلك السلسلة عمل قيد التنفيذ**. إذا انتهى التشغيل فسيُتجاهل ردك اللاحق — أعد استخدام `@mention` بدل افتراض أن الوكيل ما زال يستمع.
...
| نجح مرة، ثم جرى تجاهل رد لاحق في السلسلة                  | هذا متوقّع. ردود السلسلة بلا إشارة لا تصل إلى الوكيل إلا ما دام للسلسلة عمل نشط — أعد استخدام `@mention`.                           |
