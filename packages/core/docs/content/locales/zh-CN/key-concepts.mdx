...
(`databaseTools: "read"`)，因此代理使用 `db-schema` / `db-query` 检查应用拥有的数据，
并通过类型化应用 actions 执行写入。只有在刻意的维护界面也需要公开带范围限制的
`db-exec` / `db-patch` 时，才设置 `databaseTools: "write"`（或 `true`）；
设置 `databaseTools: "off"` / `false` 可要求所有数据访问都通过类型化应用 actions。
