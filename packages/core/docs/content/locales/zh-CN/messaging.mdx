...
- **频道提及** - 使用 @mention 开始工作。只有同一线程仍有活跃任务时，未提及机器人的回复才会继续任务。
...
推荐使用托管 OAuth。在**设置 → 消息**中选择 **Agent manifest**，下载当前主机专用的
...
订阅 `app_home_opened`、`app_context_changed`、`app_mention`、
`message.channels`、`message.groups`、`message.im` 和 `message.mpim`。
配置 `SLACK_CLIENT_ID`、`SLACK_CLIENT_SECRET` 和
`SLACK_SIGNING_SECRET` 后，在**设置 → 消息**中点击 **Add to Slack**。
...
未提及机器人的回复只会继续活跃线程；普通频道聊天不会调用代理。有限上下文包含
...
- 同一会话串里之后的普通回复，**只有在该会话串仍有运行中的工作时**才会到达代理。如果运行已结束，你的追加消息会被忽略 —— 请重新 @mention，而不要假定代理还在听。
...
| 成功过一次，之后会话串里的回复被忽略                      | 这是预期行为。没有提及的会话串回复，只在该会话串有活动工作时才会到达代理 —— 请重新 @mention。                |
