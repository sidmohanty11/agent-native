---
title: "规划插件和市场"
description: "安装 Agent-Native Plan skills（/visual-plan、/visual-recap、/visualize-repo）以及托管 Plan MCP 连接器，可作为 Claude Code 或 Codex 插件安装，也可使用通用 CLI。了解更新如何工作以及是否需要提交任何内容。"
---

# 规划插件和市场

Agent-Native **Plan** 应用以一个可安装的 bundle 提供。一次安装会添加 Plan 斜杠命令 skills，**并**连接托管的 Plan MCP 连接器，让 agent 可以生成 plans，并让 skills 直接发布到 Plan 应用中。

## 你得到什么 {#what-you-get}

一次安装即可为您提供：

- **三个 skills** — `/visual-plan`（规范入口点）、`/visual-recap` 和 `/visualize-repo`。
- **计划 MCP 连接器** — 针对 `https://plan.agent-native.com` 上的托管应用程序进行注册（MCP 端点 `https://plan.agent-native.com/mcp`，服务器名称 `plan`）。

<Diagram id="doc-block-xt4mmz" title={"三条路线，一揽子"} summary={"通用 CLI、Claude Code 插件和 Codex 插件都会安装同一组 Plan skills 以及托管的 Plan 连接器。"}>
...
默认情况下，`/visual-plan` 和 `/visual-recap` 会发布到托管的 Plan 应用 — 它们通过
...
<Diagram id="doc-block-1kkwg5h" title={"托管模式与本地文件模式"} summary={"默认情况下技能通过连接器发布；本地文件模式将 MDX 写入磁盘并通过本地主机桥进行预览。"}>
...
<div class="diagram-modes">
  ...
  <div class="diagram-card">
    <span class="diagram-pill warn">本地文件隐私</span
    ><strong>将 MDX 写入磁盘</strong
    ><small class="diagram-muted"
      >plan.mdx + canvas.mdx + prototype.mdx &rarr; localhost bridge &rarr;
      hosted Plan UI reads local source. No 数据库写入s until
...
适用于任何 host — Claude Code、Codex、Cursor、Cline、Goose、ChatGPT 自定义 MCP apps、Claude Cowork 以及任何其他 MCP-compatible host。Agent-Native CLI 会安装 Plan skills，注册托管的 Plan MCP 连接器，**并在同一步中为选定的本地客户端运行身份验证**，因此你的第一次工具调用不会遇到 OAuth 墙：

```bash
npx @agent-native/core@latest skills add visual-plan
```

这会安装 `visual-plan` 以及配套的 `visual-recap` 和 `visualize-repo` skills，然后注册 `plan` 连接器并运行身份验证（用于托管/账户支持共享的 OAuth 提示）。有用的标志：
...
`/plugin install` 会添加 Plan skills 和 **仅 URL** 的 MCP 配置（包中没有 secrets）；`/mcp` → **Authenticate** 完成 OAuth 握手。当你需要本地文件或自托管模式时，请改用通用 CLI 路径。
...
安装后，**启动新的 Codex 线程**，以便 skills 和 MCP 工具加载到会话中。该插件附带一个仅限 URL 的连接器（`[mcp_servers.plan]` → `https://plan.agent-native.com/mcp`）； `codex mcp login plan` 运行 OAuth 流程。如果您更喜欢使用一个命令来同时安装和身份验证，或者需要本地文件或自托管模式，则上面的通用 CLI 路由也适用于 Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`)。
