...
<Diagram id="doc-block-eusya0" title={"一个模式，多个后端"} summary={"应用程序代码使用框架的与方言无关的帮助程序。运行时从 DATABASE_URL 自动检测方言； unset 表示本地 SQLite 文件。"}>
  ...
  annotations={[
    {
      lines: "6-7",
      label: "仅添加剂",
      note: "`ADD COLUMN IF NOT EXISTS` 可以安全地重新运行并且永远不会丢失数据。重命名看起来像 drop+create 为 Drizzle，所以改为添加然后迁移。",
    },
    {
      lines: "13-16",
      label: "方言门控",
    ...
    },
    {
      lines: "19",
      label: "每个应用程序版本表",
      note: "每个应用程序都会跟踪自己的应用版本，因此迁移在重新启动和实例之间是幂等的。",
    },
  ]}
/>
