...
> **大多数人会把 Plan 作为 skill 安装，而不是 scaffolded app。** 一个 CLI 命令会把 `/visual-plan`、`/visual-recap`、`/visualize-repo` skills 和托管的 Plan 连接器添加到你的 coding agent — plugin 和 marketplace 路径见 [Plan plugin & marketplace](/docs/plan-plugin)。Fork Plan 模板（见 [For developers](#for-developers)）是用于自托管或基于 Plan 本身构建的次要路径。
...
<Diagram id="doc-block-yriai1" title={"两个命令，一个查看界面"} summary={"这两个命令都通过托管的 Plan MCP 连接器发布到相同的注释和评论界面。"}>
...
<div class="diagram-plan">
  ...
  <div class="diagram-node">
    编码代理<br /><small class="diagram-muted">反馈已交回</small>
  </div>
</div>
```
...
<WireframeBlock id="doc-block-gnscq3">
  <Screen
    surface="desktop"
    html={
...
该命令安装三个命令：`/visual-plan`、`/visual-recap` 和 `/visualize-repo`。

如果您使用的是直接接受 MCP 连接器 URL 的基于聊天的主机
（而不是 CLI 配置的客户端），连接托管计划连接器
`https://plan.agent-native.com/mcp` — 请参阅 [MCP Clients](/docs/mcp-clients) 以了解特定于客户端的设置。
...
生成计划或回顾会对你的编码代理的 LLM 运行一次完整的代理式会话，而不是轻量级的模板填充，因此成本会随 diff 或计划的大小以及你选择的模型而变化。

- 应用的默认聊天模型是 GPT-5.6 Luna。直接连接 Anthropic 时仍默认使用 Claude Sonnet 5。你可以随时在 **设置** 中更改模型；Haiku 是更便宜的 Claude 选项。
- 来自 GitHub Action 的自动化 PR 回顾与交互式应用分开运行，拥有自己的模型和成本调节项 — 关于 `VISUAL_RECAP_MODEL`、`VISUAL_RECAP_AGENT` 和 `VISUAL_RECAP_REASONING`，请参见[PR Visual Recap](/docs/pr-visual-recap)。
...
托管计划连接器位于 `https://plan.agent-native.com/mcp`。
切勿将共享机密放入技能文件中。
...
## 仓库可视化文档 {#visualize-repo}

当你想为代码库建立一层持久的可视化文档，而不是一次性的计划或 recap 时，使用 `/visualize-repo`。它会创建或打开本地 Plan MDX workspace，在 `agent-native.json` 中更新 `apps.visualize-repo` 的 local-files 配置，并通过同一个 localhost bridge 提供文档：

```bash
npx @agent-native/core@latest visualize-repo --open
```

如果不存在 `agent-native.json`，该命令会引导生成一个，并根据仓库结构推荐一个小的起始集合，例如 `actions/`、`app/components/`、`app/pages/`、`server/db/schema.ts`、`src/`、`packages/`、`templates/`、`docs/` 或 `content/`。你也可以指定具体的 source anchors：
...
起始文件夹默认位于 `.agent-native/visual-docs/repo-overview`。评论会保留在本地 `comments.json` 中，因此审阅者可以为 coding agent 标记章节，agent 也可以在同一次变更中同时更新代码和 MDX。运行 `visualize-repo check` 做本地 lint；当你希望 Plan renderer schema 验证 MDX 时，运行 `visualize-repo verify`。
...
- “运行 `/visualize-repo`，从我们的公共 API 操作和 schema 开始。”
...
<DataModel
  id="doc-block-1imuwd1"
  title={"规划数据模型"}
  summary={"一个计划行拥有有序部分以及评论、事件、版本、共享和内联资产。"}
  entities={[
    {
      id: "plans",
      name: "plans",
