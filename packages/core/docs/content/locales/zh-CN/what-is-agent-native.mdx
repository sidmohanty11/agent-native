...
<Diagram id="doc-block-pral9e" title={"阶梯原理"} summary={"大多数团队停在第 1 级或第 2 级。原生代理是第 3 级——一个真实的应用程序和一个共享操作界面上的真实代理。"}>

```html
<div class="diagram-ladder">
  <div class="diagram-card rung rung-3">
    <span class="diagram-pill accent">第 3 阶 · agent-native</span
    ><strong>代理 + UI 作为平等伙伴</strong
  ...
  <div class="diagram-card rung rung-2">
    <span class="diagram-pill">第 2 阶</span><strong>带工具的聊天</strong
    ><small class="diagram-muted"
      >代理可以行动，但它仍只是聊天窗口。没有仪表盘、列表或快捷键。</small
    >
  </div>
  <div class="diagram-card rung rung-1">
    <span class="diagram-pill warn">第 1 阶</span><strong>一次 LLM 调用</strong
    ><small class="diagram-muted"
...
<Diagram id="doc-block-yp0zjg" title={"一套系统，两种方式"} summary={"代理和 UI 写入相同的操作和相同的数据库。无论一个人做什么，另一个人都会看到。"}>

```html
<div class="diagram-parity">
  <div class="diagram-col">
    <div class="diagram-node">
      Human<br /><small class="diagram-muted">点击、表单、快捷键</small>
    </div>
    <div class="diagram-node">
      Agent<br /><small class="diagram-muted">自然语言 · A2A · Slack</small>
    </div>
  </div>
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">SQL 数据库</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-box">界面实时更新</div>
</div>
```
...
Agent-native 为每个用户提供相同的自定义层 - 无需离开应用程序。每个应用程序都附带一个个人**代理资源**，您（或团队中的任何人）可以：
...
请参阅 [代理资源](/docs/agent-resources) 了解完整概念。
  ...
  annotations={[
    {
      lines: "5",
      label: "工具表面",
      note: "`description` 是代理读取的内容，以决定何时将其调用为工具。",
    },
    {
      lines: "6",
      label: "类型化契约",
      note: "一个 zod `schema` 会验证来自**每个**界面的输入：代理、UI、HTTP、MCP 和 A2A。",
    },
    {
      lines: "7-10",
      label: "一次实施",
      note: "`run` 主体是唯一的事实来源。 UI 按钮和代理工具都执行此操作。",
    },
  ]}
/>
...
- [**代理资源**](/docs/agent-resources) - 由 SQL 支持的每用户自定义层（skills、内存、指令、MCP），而不是文件
