...
<Endpoint id="doc-block-15256oc" method="POST" path="/_agent-native/mcp" summary={"统一MCP网关端点"} auth="Standard remote MCP OAuth, handled by the framework. The granted-app set scopes which workspace apps the connector can reach." responses={[
  {
    "status": "200",
