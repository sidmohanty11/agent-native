  ...
  annotations={[
    {
      lines: "10",
      label: "整个大门",
      note: "一面旗帜。当它为真且调用未获批准时，循环会在 `run` 之前停止 - 模型本身永远不会达到副作用。",
    },
    {
      lines: "11-13",
      label: "run() is untouched",
      note: "处理程序保持不变。批准是由其周围的循环强制执行的，而不是由 `run` 内的任何内容强制执行。",
    },
  ]}
/>
...
<Diagram id="doc-block-96xdgg" title={"审批中断"} summary={"门控调用会在 run() 触发之前暂停回合。批准重新发出携带呼叫密钥的回合；只有这样，副作用才会发生。"}>
...
<div class="diagram-approve">
  ...
  <div class="diagram-box" data-rough>
    人在聊天中点击批准<br /><small class="diagram-muted"
      >客户端用 approvedToolCalls: [approvalKey] 重新发起本轮</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
