...
<Diagram id="doc-block-ah38lu" title={"分叉并定制"} summary={"选择一个成品，对其进行品牌化，用简单的英语对其进行改进，然后将其发送到您自己的域。"}>
...
<Diagram id="doc-block-1v5me0v" title={"为什么每用户分叉规模"} summary={"有两个想法使分叉和定制模型保持实用：代理进行维护，每个用户的定制位于 SQL 中，而不是每个用户的代码中。"}>
