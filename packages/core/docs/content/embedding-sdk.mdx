...
<Diagram id="doc-block-1cn29b5" title="The embedding membrane" summary={"The host app supplies server-side auth and live page context; Agent-Native runs the durable sidecar and reaches the open tab through client actions and host commands."}>

```html
<div class="diagram-embed">
  <div class="diagram-box" data-rough>
    <strong>Host SaaS app</strong
    ><small class="diagram-muted">your UI, your auth</small>
  </div>
  <div class="diagram-col">
    <div class="diagram-pill accent">getContext &rarr;</div>
    <div class="diagram-pill">&larr; client actions</div>
    <div class="diagram-pill">&larr; host commands</div>
  </div>
  <div class="diagram-panel center" data-rough>
    <strong>Agent-Native sidecar</strong
    ><small class="diagram-muted">durable chat · app state · extensions</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    SQL<br /><small class="diagram-muted">framework tables</small>
  </div>
</div>
```
...
```ts filename="server/plugins/agent-native.ts"
...
<Callout id="doc-block-emb1host" tone="info">
...
<Diagram id="doc-block-c9lqzk" title="Server-mediated browser-session bridge" summary={"A backend tool enqueues work; the registered tab claims it, runs it on the live page, and the result returns to the agent — so a backend/Slack/A2A agent can still touch the open tab."}>
...
<div class="diagram-bridge">
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    Live tab claims it<br /><small class="diagram-muted"
...
<Callout id="doc-block-emb1ext" tone="info">
...
- [**Agent Surfaces**](/docs/agent-surfaces) — decide between headless, chat,
  sidecar, and full-app before you start embedding.
- [**Native Chat UI**](/docs/native-chat-ui) — the chat runtime and widget
  contract the sidecar renders.
- [**Extensions**](/docs/extensions) — the SQL-backed extension system this
  SDK's portable slot is a lighter-weight alternative to.
- [**Drop-in Agent**](/docs/drop-in-agent) — mount the same agent inside an
  Agent-Native app instead of an external host.
